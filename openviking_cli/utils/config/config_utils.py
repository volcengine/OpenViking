...
def _unknown_field_message(field_path: str, valid_fields: set[str]) -> str:
    """Render one ignored config field, including a near-match suggestion."""
    message = f"Ignoring unknown config field '{field_path}'"
    parent, _, field_name = field_path.rpartition(".")
    suggestion = suggest_closest_field(field_name, valid_fields)
    if suggestion:
        suggested_path = f"{parent}.{suggestion}" if parent else suggestion
...
def warn_unknown_fields(
    *,
    ...
    for key in data:
        ...
        messages.append(_unknown_field_message(str(field_path), valid_fields))
    if messages:
        logger.warning("%s", "\n".join(messages))
