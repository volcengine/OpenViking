...
def validate_opengauss_vector_constraints(
    *,
    ...
    if access_method == "diskann" and dimension > 1536:
        raise ValueError("VectorDB opengauss DiskANN supports vector dimensions up to 1536")
    if effective_rabitq and dimension > 2000:
        raise ValueError("VectorDB opengauss RabitQ supports vector dimensions up to 2000")
    if access_method in {"hnsw", "ivfflat"} and dimension > 16000:
        raise ValueError("VectorDB opengauss HNSW/IVFFlat supports vector dimensions up to 16000")
    if effective_pq:
...
def _require_integer_range(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"openGauss {parameter_group}.{name} must be an integer") from error
    if integer_value != value or not minimum <= integer_value <= maximum:
        raise ValueError(f"openGauss {parameter_group}.{name} must be in [{minimum}, {maximum}]")
...
class OpenGaussConfig(BaseModel):
    ...
    def validate_opengauss(self):
        ...
        requested_rabitq = _require_boolean(build, "enable_rabitq", parameter_group="build_params")
        if quantization == "pq":
            requested_pq = True
        ...
        if access_method == "hnsw":
            m = _require_integer_range(build, "m", 2, 100, parameter_group="build_params")
            ...
            if effective_ef < 2 * effective_m:
                raise ValueError("openGauss hnsw build_params.ef_construction must be >= 2 * m")
            ...
            _require_integer_range(build, "lists", 1, 32768, parameter_group="build_params")
            ...
            _require_integer_range(build, "index_size", 16, 1000, parameter_group="build_params")
        ...
        if requested_pq:
            _require_integer_range(build, "pq_m", 1, 2000, parameter_group="build_params")
            _require_integer_range(build, "pq_ksub", 1, 256, parameter_group="build_params")
        else:
            if "pq_m" in build:
                _require_integer_range(build, "pq_m", 1, 2000, parameter_group="build_params")
            if "pq_ksub" in build:
            ...
            if "ivfpq_kreorder" in search:
                raise ValueError("openGauss search_params.ivfpq_kreorder requires IVF-PQ")
...
class VectorDBBackendConfig(BaseModel):
    ...
    def validate_config(self):
        ...
        if self.backend in {"local", "cuvs"}:
            ...
            access_method, quantization = resolve_opengauss_index_spec(self.opengauss.index_type)
