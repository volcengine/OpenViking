...
function formatOVSearchRows(result) {
...
function formatOVSearchText(query, uri, result) {
    if ((result.total ?? 0) <= 0) {
...
function validateOpenVikingUri(toolName, uri) {
    if (!uri) {
    ...
    if (uri.endsWith("...") || uri.includes("…")) {
        throw new Error(`${toolName} received a truncated display URI. Use the exact full viking:// URI from ov_search details/results; do not shorten it with ... or ….`);
    }
}
function formatOVListEntry(entry) {
    ...
    const summary = typeof item.abstract === "string" && item.abstract.trim()
...
function formatOVListText(uri, entries) {
    if (entries.length === 0) {
...
export function createOpenVikingQueryRuntime(deps) {
    const toTraceResult = (item, resultType) => ({
        uri: item.uri,
    ...
    const readOpenVikingContent = async (input, agentId) => {
        ...
        return {
            content: [{ type: "text", text: formatOVReadText(uri, text) }],
            details: {
                action: "read",
                uri,
                chars: text.length,
    ...
    const multiReadOpenVikingContent = async (input, agentId) => {
        ...
        return {
            content: [{ type: "text", text: formatOVMultiReadText(results) }],
            details: {
                action: "multi_read",
    ...
    const listOpenVikingDirectory = async (input, agentId) => {
        const uri = input.uri.trim();
        ...
        return {
            content: [{ type: "text", text: formatOVListText(uri, entries) }],
            details: {
                action: "listed",
                uri,
                recursive: input.recursive ?? false,
    ...
    const searchOpenViking = async (input, agentId, traceCtx) => {
        ...
        if (searchUri) {
            ...
            const runSearch = async (targetUri) => {
                const started = Date.now();
                try {
        ...
        await deps.traceRecorder?.recordAndFlush?.({
            ...
            resourceTypes: [...new Set(searches.map((search) => search.resourceType).filter((resourceType) => resourceType !== "archive"))],
        ...
        return {
            content: [{ type: "text", text: formatOVSearchText(query, searchUri, result) }],
            details: {
                action: "searched",
                query,
                uri: searchUri,
