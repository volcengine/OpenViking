...
export function registerOpenVikingArchiveTools(deps) {
    deps.registerTool((ctx) => ({
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                const traceResults = (result.matches ?? []).slice(0, deps.traceRecallMaxResultsPerSearch).map((match) => ({
                    uri: match.uri,
                    resourceType: "archive",
                    abstractPreview: previewText(match.content, deps.traceRecallPreviewChars),
                    resultType: "archive_match",
                }));
                const recordArchiveTrace = async (displayed) => {
