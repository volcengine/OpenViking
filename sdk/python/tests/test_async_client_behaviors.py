...
async def test_retrieval_forwards_include_timestamps(method_name, kwargs, expected_path):
    client = AsyncHTTPClient(url="http://localhost:1933")
    client._request = AsyncMock(return_value=object())
    client._handle_response_data = lambda _response: {"result": {}}

    await getattr(client, method_name)("hello", options={"include_timestamps": True}, **kwargs)

    assert client._request.await_args.args == ("POST", expected_path)
    assert client._request.await_args.kwargs["json"]["include_timestamps"] is True
