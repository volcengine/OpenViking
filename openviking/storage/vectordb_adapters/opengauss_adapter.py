...
class OpenGaussCollection(ICollection):
    ...
    def _apply_parallel_workers(self, index_meta: Dict[str, Any]) -> None:
        parallel_workers = int(index_meta.get("build_params", {}).get("parallel_workers", 0) or 0)
    ...
    def create_index(self, index_name: str, meta_data: Dict[str, Any]) -> IIndex:
        ...
        runtime_config = OpenGaussConfig(
            mode="distributed" if getattr(self, "_distributed", False) else "standalone",
        ...
        if parallel_workers is not None:
            validated_meta["build_params"]["parallel_workers"] = runtime_config.parallel_workers
        ...
        if distance == "l1" and (runtime_config.access_method != "hnsw" or effectively_quantized):
