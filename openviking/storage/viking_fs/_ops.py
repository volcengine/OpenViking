...
class _OpsMixin:
    ...
    async def rm(
        ...
        try:
            ...
            # The AGFS listing is gone, so ``_collect_uris`` yields nothing and the
            # exact-match delete below would only drop the target record, leaving
            # every descendant behind as an orphan (#3064). Discover the
            # descendants from the vector index instead - the same source that
            # ``_estimate_deleted_count`` already counts them from.
            real_ctx = self._ctx_or_default(ctx)
            uris_to_delete = (
                await self._collect_orphan_uris(target_uri, ctx=real_ctx) if recursive else []
            )
