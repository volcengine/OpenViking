...
class AddResourceProcessor(DequeueHandlerBase):
    ...
    def _requeue_lock_handoff(self, msg: AddResourceMsg, exc: Exception) -> Optional[ProcessResult]:
        ...
        logger.warning(
            "[AddResource] Requesting retry for task %s after lock handoff failure: %s",
            msg.task_id,
            exc,
        )
        return ProcessResult.requeue(payload, error=str(exc))
    ...
    async def _process(self, msg: AddResourceMsg, data: Dict[str, Any]) -> ProcessResult:
        ...
        if replay_result is None:
            try:
                ...
                retry = self._requeue_lock_handoff(msg, exc)
        ...
        with (
            ...
            try:
                ...
                queue_status = await tracker.queue_status(
                    msg.task_id,
                    account_id=ctx.account_id,
                    user_id=ctx.user.user_id,
                )
                result.setdefault("queue_status", queue_status)
                if replay_result is None:
                    embedding_status = queue_status.get("Embedding", {})
                    result["context_count"] = int(embedding_status.get("processed", 0) or 0)
    ...
    async def on_dequeue(self, data: Optional[Dict[str, Any]]) -> ProcessResult:
        ...
        future: concurrent.futures.Future[ProcessResult] = asyncio.run_coroutine_threadsafe(
