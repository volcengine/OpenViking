...
def _shared_upload_uri(cfg: TempUploadConfig, ctx: RequestContext, upload_id: str) -> str:
    return f"{_shared_prefix(cfg)}/{ctx.user.user_space_name()}/{ctx.account_id}/{upload_id}"
...
class TempUploadStore:
    ...
    async def mark_failed(self, resolved: ResolvedTempUpload, ctx: RequestContext) -> None:
        ...
        meta["state"] = "uploaded"
        meta["consuming_started_at"] = None
    ...
    async def mark_consumed(self, resolved: ResolvedTempUpload, ctx: RequestContext) -> None:
        ...
        await get_viking_fs().rm(uri, recursive=True, ctx=ctx, lock_handle=resolved.lock_handle)
    ...
    async def try_cleanup_invalid_or_expired(
        ...
        uri = _shared_upload_uri(self.temp_cfg, ctx, shared_id)
        with suppress(Exception):
            await get_viking_fs().rm(uri, recursive=True, ctx=ctx)
    ...
    async def _save_shared(self, upload_file: Any, ctx: RequestContext) -> str:
        if not self.temp_cfg.shared_enabled:
            raise InvalidArgumentError("upload_mode=shared is not enabled on this server.")
        ...
        meta = {
            ...
            "created_at": now,
            "updated_at": now,
            "expire_at": now + self.temp_cfg.shared_ttl_seconds,
            "consuming_started_at": None,
        }

        try:
            with open(temp_path, "rb") as f:
                content = f.read()
            await vfs.write_file_bytes(content_uri, content, ctx=ctx)
            await vfs.write_file(meta_uri, json.dumps(meta, ensure_ascii=False), ctx=ctx)
            return temp_file_id
        except Exception:
            with suppress(Exception):
    ...
    async def _resolve_shared(
        ...
        vfs = get_viking_fs()
        if not await vfs.exists(content_uri, ctx=ctx):
            await self.try_cleanup_invalid_or_expired(temp_file_id, ctx)
            raise PermissionDeniedError("Temporary upload is invalid: content missing.")

        lock_path = vfs._uri_to_path(_shared_upload_uri(self.temp_cfg, ctx, upload_id), ctx=ctx)
        ...
        try:
            meta = await self._read_shared_meta(upload_id, ctx)
            now = int(time.time())
            if meta.get("state") == "consuming":
                started = int(meta.get("consuming_started_at") or 0)
                if started and now - started > self.temp_cfg.shared_consuming_timeout_seconds:
                    meta["state"] = "uploaded"
                    meta["consuming_started_at"] = None
                    meta["updated_at"] = now
            ...
            os.close(fd)
            try:
                content = await vfs.read_file_bytes(content_uri, ctx=ctx)
                with open(temp_path, "wb") as f:
                    f.write(content)
            except Exception:
                with suppress(FileNotFoundError):
    ...
    async def _read_shared_meta(self, upload_id: str, ctx: RequestContext) -> dict[str, Any]:
        meta_uri = _shared_meta_uri(self.temp_cfg, ctx, upload_id)
        try:
            raw = await get_viking_fs().read_file(meta_uri, ctx=ctx)
            data = json.loads(raw)
    ...
    async def _write_shared_meta(
        ...
        meta_uri = _shared_meta_uri(self.temp_cfg, ctx, upload_id)
        await get_viking_fs().write_file(meta_uri, json.dumps(meta, ensure_ascii=False), ctx=ctx)
    ...
    def _validate_shared_meta(
        ...
        if meta.get("account") != ctx.account_id or meta.get("user") != ctx.user.user_id:
            raise PermissionDeniedError("Temporary upload does not belong to current account/user.")
        if int(meta.get("expire_at") or 0) <= now:
            raise PermissionDeniedError("Temporary upload has expired.")
