...
Uses OpenViking's existing LLM configuration for intelligent filtering and summarization.
"""
import json
import re
...
class KnowledgeExtractor:
    ...
    def __init__(self, llm_service=None):
        """
        Args:
            llm_service: Optional LLM service. If None, will use OpenViking's default.
        """
        self._llm_service = llm_service

    def _get_llm_service(self):
        """Lazy-load the LLM service from OpenViking if not provided."""
        if self._llm_service is None:
            from openviking.server.dependencies import get_service
            self._llm_service = get_service().llm
