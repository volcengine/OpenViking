...
def candidate_retry_draft(draft: Any, *, target_names: set[str]) -> Any:
    ...
    for field_name in ("experiences", "write_uris", "edit_uris", "upsert_operations"):
        ...
        selected = [
            value
            for value in values
            if _draft_candidate_names(value).intersection(target_names)
        ]
        if selected:
            matched = True
...
class ExperienceSkillReadabilityGate:
    ...
    async def evaluate(self, target: GateTarget) -> GateDecision | None:
        ...
        return GateDecision(
            ...
            retriable=True,
            repair_prompt=(
                "Repair only these observed issues: " + "; ".join(repair_steps) + ". Put the "
                "complete four-section Markdown body in the `constraint` field (or `content` "
                "if that is the only available field). Preserve already-correct, evidence-backed "
                "content and do not add trigger_code."
            ),
        )
...
def _compound_repair_actions(reminder: str) -> list[str]:
    ...
    return list(
        dict.fromkeys(
            match.group(0).lower()
            for match in _COMPOUND_REPAIR_ACTION_RE.finditer(reminder)
        )
    )
...
def _independent_experience_concerns(content: str) -> list[str]:
    ...
    return [
        name
        for name, pattern in _INDEPENDENT_EXPERIENCE_CONCERN_PATTERNS
...
def _experience_name_polarity_contradiction(
    ...
    pairs = (
        (r"^skip\b", r"(?i)\b(?:do\s+not|don't|never|must\s+not|should\s+not)\s+skip\b|\bavoid\s+skipping\b"),
        (r"^omit\b", r"(?i)\b(?:do\s+not|don't|never|must\s+not|should\s+not)\s+omit\b|\bavoid\s+omitting\b"),
        (r"^ignore\b", r"(?i)\b(?:do\s+not|don't|never|must\s+not|should\s+not)\s+ignore\b|\bavoid\s+ignoring\b"),
