...
class ResourceService:
        ...
        **kwargs,
        ...
        try:
            ...
            if not wait:
                ...
                root_uri = result.get("root_uri", "")
                task = await task_tracker.create_async(
                    "add_resource",
                ...
                if telemetry_id:
                    ...
                    await task_tracker.start_async(
                        task.task_id, account_id=ctx.account_id, user_id=ctx.user.user_id
                    )
                    await task_tracker.complete_async(
                        task.task_id,
                        {"root_uri": root_uri},
                        account_id=ctx.account_id,
                        user_id=ctx.user.user_id,
    ...
    async def _monitor_queue_processing(
        ...
        task_tracker = get_task_tracker()
        request_wait_tracker = get_request_wait_tracker()
        await task_tracker.start_async(task_id, account_id=account_id, user_id=user_id)
        try:
            ...
            await task_tracker.fail_async(task_id, str(exc), account_id=account_id, user_id=user_id)
        finally:
