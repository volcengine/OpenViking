...
class TaskTracker:
    ...
    async def reap_stale_active(
        ...
        for task_id in candidates:
            # Serialize with lifecycle mutations on the same task: a worker may
            # have registered a live asyncio task, resumed, or finished between
            # the snapshot above and this iteration.
            async with self._task_locks.acquire(task_id):
                previous = self._cached_task(task_id)
                if previous is None:
                    continue
                if previous.status not in (TaskStatus.PENDING, TaskStatus.RUNNING):
                    continue
                if self.has_live_task(task_id):
                    continue
                if (now - previous.updated_at) <= threshold:
                    continue
                ...
                task.error = _sanitize_error(
                    ...
                    f"{int(now - previous.updated_at)}s (likely process restart); "
                    "marked failed by startup reconciliation"
                )
                task.updated_at = now
                await self._persist_and_publish("update", task, previous=previous)
                reaped += 1
        if reaped:
    ...
    async def _hydrate_from_store(self) -> None:
        ...
        records = [
            ...
            for payload in await self._store_io.run("list_all", list_all)
        ]
        with self._lock:
