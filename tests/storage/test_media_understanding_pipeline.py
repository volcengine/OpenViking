...
class RecordingVLM:
    ...
    def supports_media(self, *, media_type, filename, size_bytes):
        ...
        return (
            self.available
            and self.media_enabled
            and Path(filename).suffix.lower() in extensions.get(media_type, set())
    ...
    async def get_media_completion_async(self, prepare_media=None, **_kwargs):
        ...
        return self.media_backend.result
...
def _config(media_client, vlm, *, configure_audio=True):
    ...
    vlm.media_backend = media_client
    vlm.media_enabled = configure_audio
...
async def test_success_flows_through_overview_write_and_leaf_embedding(monkeypatch):
    ...
    overview, abstract, vector_task = await _run_overview(processor, fs, result, "meeting.mp3")
    leaf_message = await _vectorize_leaf(monkeypatch, config, fs, "meeting.mp3", result)
...
async def test_empty_media_summary_uses_generic_directory_and_filename_leaf(
    ...
    overview, abstract, vector_task = await _run_overview(SemanticProcessor(), fs, result, filename)
