...
``_collect_uris`` (which yields nothing for a missing path) plus
...
class TestRmOrphanIndexCleanup:
    ...
    async def test_recursive_rm_cleans_orphan_children_from_index(self) -> None:
        ...
        # The AGFS path is gone, so the directory listing yields nothing.
        ...
        assert scope.path.rstrip("/") == target_uri
