    ...
    def test_quick_start_script_execution(self):
        ...
        def generate_pseudo_completion(prompt: str, images=None, **kwargs):
            ...
            if "generate a summary for the following file" in prompt_lower:
                ...
                return (f"Summary of {filename}: This file contains documentation or code "
                        f"relevant to {filename}. Key topics include configuration, usage, and API details.")

            # 2. Directory Overview Task
            elif "generate an overview document based on the following directory content" in prompt_lower:
                import re
        ...
        mock_vlm.get_completion_async.side_effect = lambda prompt, *args, **kwargs: async_return(generate_pseudo_completion(prompt))
        
        mock_vlm.get_vision_completion.side_effect = lambda prompt, images, **kwargs: generate_pseudo_completion(prompt, images)
        mock_vlm.get_vision_completion_async.side_effect = lambda prompt, images, **kwargs: async_return(generate_pseudo_completion(prompt, images))
        ...
        with patch.dict(os.environ, env_override), \
             patch('openviking.utils.config.embedding_config.EmbeddingConfig.get_embedder', return_value=mock_embedder), \
             patch('openviking.utils.config.vlm_config.VLMConfig.get_vlm_instance', return_value=mock_vlm):
