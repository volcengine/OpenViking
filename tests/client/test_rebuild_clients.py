...
async def test_async_openviking_reindex_forwards_to_local_client():
    client = AsyncOpenViking.__new__(AsyncOpenViking)
    client._ensure_initialized = AsyncMock()
    client._client = SimpleNamespace(reindex=AsyncMock(return_value={"status": "completed"}))
    ...
    client._client.reindex.assert_awaited_once_with(
...
async def test_async_openviking_memory_graph_health_forwards_to_local_client():
    client = AsyncOpenViking.__new__(AsyncOpenViking)
    client._ensure_initialized = AsyncMock()
    client._client = SimpleNamespace(memory_graph_health=AsyncMock(return_value={"healthy": True}))
    ...
    client._ensure_initialized.assert_awaited_once()
    client._client.memory_graph_health.assert_awaited_once_with(
        "viking://agent/default/memories",
        node_limit=123,
        sample_limit=4,
    )
...
def test_sync_openviking_reindex_forwards_to_async_client():
    ...
    reindex_coro = object()
    client._async_client = SimpleNamespace(reindex=Mock(return_value=reindex_coro))
    with patch(
    ...
    client._async_client.reindex.assert_called_once_with(
        uri="viking://resources/demo",
        mode="semantic_and_vectors",
        wait=True,
    )
...
def test_sync_openviking_memory_graph_health_forwards_to_async_client():
    ...
    client = SyncOpenViking.__new__(SyncOpenViking)
    client._async_client = SimpleNamespace(memory_graph_health=Mock(return_value=graph_health_coro))
    with patch(
    ...
    client._async_client.memory_graph_health.assert_called_once_with(
        "viking://agent/default/memories",
        node_limit=321,
        sample_limit=5,
    )
