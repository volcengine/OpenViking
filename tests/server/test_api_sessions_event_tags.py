...
async def test_session_event_tags_preserve_free_form_values(client: httpx.AsyncClient):
    response = await client.post(
        "/api/v1/sessions",
        json={"memory_extraction_config": {"events": {"tags": ["missing-equals", "channel=web"]}}},
    )

    assert response.status_code == 200
    assert _event_tags(response) == ["missing-equals", "channel=web"]
