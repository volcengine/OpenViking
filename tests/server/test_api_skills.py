...
async def test_skills_api_manages_peer_skills_with_root_uri(client):
    peer_root_uri = "viking://user/default/peers/alice/skills"
    add_response = await client.post(
        "/api/v1/skills",
        json={
            "data": _skill_md("peer-skill", "Peer skill"),
            "to": f"{peer_root_uri}/peer-skill",
            "wait": True,
        },
    )
    assert add_response.status_code == 200, add_response.text
    assert add_response.json()["result"]["root_uri"] == f"{peer_root_uri}/peer-skill"

    default_list_response = await client.get("/api/v1/skills")
    assert default_list_response.status_code == 200, default_list_response.text
    assert all(
        skill["name"] != "peer-skill"
        for skill in default_list_response.json()["result"]["skills"]
    )

    peer_list_response = await client.get(
        "/api/v1/skills",
        params={"root_uri": peer_root_uri},
    )
    assert peer_list_response.status_code == 200, peer_list_response.text
    assert any(
        skill["name"] == "peer-skill"
        for skill in peer_list_response.json()["result"]["skills"]
    )

    show_response = await client.get(
        "/api/v1/skills/peer-skill",
        params={"root_uri": peer_root_uri, "level": 2},
    )
    assert show_response.status_code == 200, show_response.text
    assert show_response.json()["result"]["root_uri"] == f"{peer_root_uri}/peer-skill"

    find_response = await client.post(
        "/api/v1/skills/find",
        json={"query": "peer", "root_uri": peer_root_uri, "limit": 5},
    )
    assert find_response.status_code == 200, find_response.text
    assert find_response.json()["result"]["root_uri"] == peer_root_uri
    ...
    assert delete_response.status_code == 200, delete_response.text
    assert delete_response.json()["result"]["root_uri"] == f"{peer_root_uri}/peer-skill"


async def test_skills_api_rejects_skill_target_under_resources(client):
    response = await client.post(
        "/api/v1/skills",
        json={
            "data": _skill_md("wrong-root-skill", "Wrong root skill"),
            "parent": "viking://user/default/peers/alice/resources",
            "wait": True,
        },
    )
