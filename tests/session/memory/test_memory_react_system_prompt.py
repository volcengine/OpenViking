...
class TestProviderInstruction:
    ...
    def test_instruction_contains_read_before_edit_instructions(self):
        ...
        assert "Before editing ANY existing memory file, you MUST first read its complete content" in instruction
        assert "ONLY read URIs that are explicitly listed in ls tool results or returned by previous tool calls" in instruction
