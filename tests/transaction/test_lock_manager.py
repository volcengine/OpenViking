...
class TestLockManagerBasic:
    ...
    async def test_explicit_none_timeout_passes_through_as_infinite_wait(self, lm):
        ...
        lm._path_lock.acquire_subtree = AsyncMock(return_value=True)

        ok = await lm.acquire_subtree(handle, "/local/test", timeout=None)

        assert ok is True
        lm._path_lock.acquire_subtree.assert_awaited_once_with("/local/test", handle, timeout=None)
