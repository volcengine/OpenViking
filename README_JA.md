...
### OpenViking Helper for macOS / Windows（Beta）

**OpenViking Helper** は OpenViking ユーザー向けのデスクトップアプリです。ローカル Agent の接続、セッショントレース、メモリ、スキルを可視化し、接続状態の確認とローカルコンテキストの OpenViking への同期を容易にします。

主な機能：

- **ローカル Agent 接続の可視化**：OpenViking CLI、Claude Code、Codex、Cursor、Trae、OpenCode を検出し、対応している plugin、MCP、Hook、CLI 接続を設定します。
- **セッショントレースの確認**：Claude Code、Codex、Trae のセッションを解析し、OpenViking の recall、Prompt 注入、MCP 呼び出し、capture、commit イベントを表示します。
- **ローカルメモリとスキル管理**：ローカルの memory / rule ファイルと `SKILL.md` スキルを確認し、OpenViking に同期します。

> OpenViking Helper は現在 Beta 版で、macOS と Windows x64 をサポートしています。
...
OpenVikingにはメモリの自己反復ループが組み込まれています。セッションをコミットすると、システムはタスクの実行結果とユーザーフィードバックを非同期に分析し、メモリポリシーに従って現在のユーザーまたは Peer 名前空間のメモリを更新します。
