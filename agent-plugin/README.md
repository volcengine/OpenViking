...
servers/shared/                      # vendored from examples/memory-plugin-shared/lib (do not edit)
...
Config file changes are picked up by the running proxy without a restart. Debugging: set `OPENVIKING_DEBUG=1` to log JSON lines to `~/.openviking/logs/agent-plugin.log` (path override: `OPENVIKING_DEBUG_LOG`).
