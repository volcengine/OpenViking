...
その他のプラットフォーム（例：Linux ARM64、FreeBSD）では、`pip`によるインストール時にソースから自動コンパイルされます。[前提条件](#前提条件)がインストールされていることを確認してください。
...
OpenVikingはAGFSに対してデフォルトで`binding-client`モードを使用し、プリビルドされた共有ライブラリが必要です。**AGFS（Go）**コードや**C++拡張**を変更した場合、またはプリビルドライブラリが見つからない場合は、再コンパイルと再インストールが必要です。プロジェクトルートで以下のコマンドを実行してください：

```bash
uv pip install -e . --force-reinstall
```

このコマンドにより`setup.py`が再実行され、AGFSおよびC++コンポーネントのコンパイルがトリガーされます。
...
│   │
│   ├── core/             # コアデータモデル
│   │   ├── context.py    # Contextベースクラス
│   │   └── directories.py # ディレクトリ定義
│   │
│   ├── parse/            # リソースパーサー
│   │   ├── parsers/      # パーサー実装
│   │   ├── tree_builder.py
...
│   │   ├── bootstrap.py  # エントリポイント（openviking-server）
│   │   └── routers/      # APIルーター
│   │
│   ├── storage/          # ストレージレイヤー
...
共通フィクスチャは`tests/conftest.py`に定義されており、`client`（初期化済み`AsyncOpenViking`）、`uninitialized_client`、`temp_dir`などが含まれます。
...
| **mainへのプッシュ** | `ci.yml` | **Test Full**（全OS：Linux/Win/Mac、全Pyバージョン：3.10-3.13）と**CodeQL**（セキュリティスキャン）を実行。mainブランチの安定性を保証。（**02. Main Branch Checks**として表示） |
...
サポートされているすべてのプラットフォーム（Linux/Mac/Win）とPythonバージョン（3.10-3.13）で完全なテストスイートを実行。手動トリガー時にカスタムマトリックス設定をサポートします。

*   **入力**:
    *   `os_json`: 実行するOSのリスト（デフォルト：`["ubuntu-24.04", "macos-14", "windows-latest"]`）。
    *   `python_json`: Pythonバージョンのリスト（デフォルト：`["3.10", "3.11", "3.12", "3.13"]`）。
