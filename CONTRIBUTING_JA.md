...
- ホストファイルの取り込み、読み取り、検索、プレビュー、エクスポートのために、サーバーのローカルパスや`file://` URLを受け付けないでください。リモートクライアントは、認可されたリソースURI、またはリクエスト主体が所有するアップロードファイルを使用する必要があります。これらの操作やエラーレスポンスを通じて、サーバー設定、認証情報、プロセス環境に関するファイル、他テナントのデータを公開しないでください。
- パスを解決して実際のファイル操作を行うレイヤーで、許可範囲への制限と認可を適用してください。解決後の対象を検証し、パストラバーサル、エンコードされたパス、絶対パス、シンボリックリンク、アーカイブ内のエントリを考慮してください。元のパス文字列や呼び出し元の初期ディレクトリの確認だけでは不十分です。アクセスするすべてのファイルを許可された境界内に保ってください。
...
コマンドや引数のインジェクション、任意のサーバーファイルへのアクセス、意図した実行境界やテナント境界からの逸脱は、セキュリティ問題です。疑わしい問題は[SECURITY.md](SECURITY.md)に記載された非公開の窓口から報告してください。公開ドキュメントやPRでは必要な境界を説明し、未修正の問題の悪用方法や機密データを公開しないでください。
