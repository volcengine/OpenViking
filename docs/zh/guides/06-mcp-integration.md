...
连接后，OpenViking MCP 端点暴露 5 个工具，覆盖记忆的完整生命周期：

| 工具 | 说明 | 主要参数 |
|------|------|----------|
| `search` | 语义搜索记忆、资源和技能 | `query`, `target_uri`(可选), `limit` |
| `read` | 读取一个或多个 `viking://` URI 的内容 | `uris`（单个或数组） |
| `store` | 存储信息到长期记忆（触发记忆提取） | `text`, `role` |
| `forget` | 删除指定记忆（按 URI 或搜索查询） | `uri` 或 `query` |
