...
这个开关当前只感知显存，不感知每条查询的预期延迟。高选择性 scalar filter 下，
native scalar index 可以先显著缩小 distance candidate set，而缓存后的 cuVS
brute-force 仍接近 full-matrix scan，因此 native 仍可能更快。按 filter selectivity
做 adaptive per-query routing 是独立的后续优化。
