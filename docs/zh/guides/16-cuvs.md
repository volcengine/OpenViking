...
新的 immutable GPU snapshot。snapshot dirty 期间查询直接使用当前 native index，
不会把 GPU build 时间转化成请求排队时间。worker 只在 record generation 仍匹配时
原子提交 label layout 和 GPU snapshot；过期 build 会被丢弃，并只重建最新一代。

## GPU 显存占用

默认 GPU shadow 是 float32，因此 brute-force 的主要常驻 payload 为
`N * dimension * 4` bytes。显式设置 `dtype: "float16"` 后，vector payload
