...
`storage.vectordb.sparse_weight` 就自动出现。OpenViking 为 `volcengine`、
`vikingdb` 和 `local_bm25` 实现了 sparse embedding provider；其中 `volcengine`
和 `vikingdb` 还提供 hybrid embedding provider。OpenAI 兼容接口、Ollama 和内置
`local` provider 目前都只支持 dense。因此，自托管的 `/v1/embeddings` 不会被
自动当成 sparse 接口，OpenViking 也不会额外探测 `/v1/embeddings/sparse` 路由。

当 provider 只返回 dense vector 时，OpenViking 不会自动补充 BM25 或其他
sparse-vector 兜底。若要启用混合检索，需要配置受支持的 sparse 或 hybrid
