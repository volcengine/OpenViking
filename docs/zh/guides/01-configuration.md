...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `backend` | str | VectorDB 后端类型: 'local'（基于文件）, 'http'（远程服务）, 'volcengine'（云上VikingDB）, 'vikingdb'（私有部署）, 'oceanbase'（OceanBase） | "local" |
...
| `oceanbase` | object | 'oceanbase' 类型的 OceanBase 配置 | - |
...
使用 [OceanBase](https://www.oceanbase.com/) 作为向量库后端。安装与配置详见 [OceanBase 集成指南](./06-oceanbase-integration.md)。
