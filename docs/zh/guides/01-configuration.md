...
将指定扩展名的资源路由到第三方 Understanding API，而不是内置解析器注册表。该路径是显式启用的：只有当 `parser_api.enable` 为 `true`，并且资源扩展名命中 `parser_api.extensions` 时，OpenViking 才会使用它。
...
对于本地文件，OpenViking 会先通过 Files API 上传文件，再用返回的 `file_id` 提交 Responses API 请求。对于 URL，OpenViking 会直接把 URL 提交给 Responses API。如果 `host` 不是以 `/api/v3` 结尾，OpenViking 会在调用 `/files` 和 `/responses` 前自动追加 `/api/v3`。

| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `enable` | bool | 将命中扩展名的资源路由到 Understanding API | `false` |
| `extensions` | list[str] | 需要路由的文件扩展名。会归一化为小写，可带或不带开头的点 | `[]` |
| `host` | str | 带 scheme 的 API 主机，例如 `https://api.example.com`；`enable` 为 `true` 时必填 | `""` |
| `api_key` | str | 调用 Files 和 Responses API 使用的 Bearer token；`enable` 为 `true` 时必填 | `""` |
| `enable_resumable_upload` | bool | 本地文件超过 `upload_simple_max_bytes` 时允许使用分片/断点续传上传 | `false` |
| `upload_simple_max_bytes` | int | 简单上传路径允许的最大文件大小 | `536870912` |
| `upload_part_size_bytes` | int | 分片上传的 part 大小 | `8388608` |
| `http_timeout_seconds` | float | 创建请求和轮询状态时的 API 超时时间 | `10.0` |
| `response_timeout_seconds` | int | 轮询 Responses API 任务的最长等待时间 | `1800` |
| `poll_interval_ms` | int | Responses API 状态检查的轮询间隔 | `3000` |
...
本文档上方各配置段（embedding、vlm、parser_api、rerank、storage）均属于 `ov.conf`。SDK 嵌入模式和服务端共用此文件。
