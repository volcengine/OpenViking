...
| `threshold` | float | 相关性阈值，判定方式为 `score > threshold`。当 `normalize_scores` 为 `true` 时，分数会被 sigmoid 映射到 `[0, 1]`，因此 `[0, 1]` 区间内的阈值可在不同精排模型间通用；当为 `false` 时，阈值在 provider 的原始分数空间中比较，对于基于 logit 的 cross-encoder（BGE/MiniLM）该空间可能**无界**。默认：`0.1`（在开启归一化或 provider 本身输出有界分数时可通用）。 |
| `normalize_scores` | bool | 对精排分数应用 sigmoid 变换，使 `threshold` 在不同 provider 间可比。Cross-encoder 精排模型（如通过 OpenAI 兼容/LiteLLM 接口调用的 BGE/MiniLM）可能返回无界的原始 logit，此时固定的 `0.1` 阈值毫无意义；而 VikingDB/Cohere 返回 `[0, 1]` 分数。设为 `true` 时，分数会在阈值判定与融合前被映射到 `[0, 1]`（闭区间——极端 logit 会饱和到恰好 `0.0`/`1.0`）。对于本身已返回归一化 `[0, 1]` 分数的 provider（如 VikingDB）应保持关闭，否则二次归一化会扭曲分数。默认：`false`（不改变 provider 原始分数）。 |
