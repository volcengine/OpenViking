...
| `account` | trusted 模式或 password 认证使用的 account 身份 | `null` |
| `user` | trusted 模式或 password 认证使用的 user 身份 | `null` |
| `password` | 未配置 API Key 时，配合 `account` + `user` 使用的密码认证凭证 | `null` |
...
如果使用 password 认证，请在 `ovcli.conf` 中配置 `account`、`user` 和
`password`。
...
`api_key` 模式使用 API Key 或 `account` + `user` + `password` 认证，也是默认模式；`trusted` 模式信任上游网关或受信调用方注入的 `X-OpenViking-Account` / `X-OpenViking-User` 请求头。

在 `api_key` 模式下配置 `root_api_key` 后，服务端启用正式多租户认证，并通过 Admin API 创建工作区、用户 key 和用户密码。在 `trusted` 模式下，普通请求不需要先注册 user key；每个请求都会根据注入的身份头解析成 `USER`。只有在 `auth_mode = "api_key"` 且未配置 `root_api_key` 时，服务端才会进入开发模式。
...
| `api_key_hashing.enabled` | bool | 是否对 API key 字段启用 Argon2id 单向哈希（与文件级 `enabled` 独立控制；用户 password 始终 hash 存储），详见 [加密指南](./08-encryption.md) | `false` |
