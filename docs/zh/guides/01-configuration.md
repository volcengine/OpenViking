...
| `temp_upload.ttl_seconds` | int | local 和 shared 临时上传文件共用的保留时间（秒）。每次对应模式的上传会清理超过此时间的文件；shared 仅清理当前 account，并只需一次上传根目录列举。完整 shared 上传按 `.meta` 的修改时间过期，孤儿 `.content` 对象按自身修改时间过期；必须大于 `0`。 | `43200`（12 小时） |
