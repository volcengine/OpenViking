...
| `enable_shared_collection_handle` | bool | 仅对 `local` 后端生效；开启后复用同一路径的本地嵌入式句柄，默认关闭以保持原多路模式 | false |
...
如果你在容器或高并发场景下遇到本地嵌入式向量库 `LOCK` 冲突，可以仅对 local backend 显式开启共享句柄模式：
...
```json
{
  "storage": {
    "vectordb": {
      "backend": "local",
      "enable_shared_collection_handle": true
    }
  }
}
```
