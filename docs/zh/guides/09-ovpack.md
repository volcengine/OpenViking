...
my-project/_._ovpack/dense.f32              # 仅 --include-vectors 且存在可导出向量时出现
my-project/_._ovpack_manifest.json
```

OpenViking 在 ZIP 中会把普通点文件转义为 `_._` 前缀：

| OpenViking 文件 | ZIP 内路径 |
| --- | --- |
...
8. `_._ovpack/index_records.jsonl` 和可选 `_._ovpack/dense.f32` 必须匹配 manifest 中的 hash、数量和维度。
