...
OpenViking 使用两层 API Key 体系。在 `api_key` 模式下，租户用户也可以用
`account` + `user` + `password` 认证；如果同时传 API Key 和 password，API Key
优先生效。
...
|------|--------------------|----------|--------------|
| API Key 模式 | `"api_key"` | API Key，或 `account` + `user` + `password`；数据归属从凭证解析 | 标准多租户部署 |
...
| 方法 | 用途 |
|------|------|
| `resolve_identity(request, api_key, x_openviking_account, x_openviking_user, x_openviking_password)` | 将凭据解析为 `ResolvedIdentity`。 |
...
普通读写、检索、会话等数据请求在 `api_key` 和 `trusted` 两种模式下都不依赖 Admin API 预注册。Admin API 仍然负责创建 account、注册用户、修改角色、签发 user key 以及设置用户密码。密码在服务端以 hash 形式保存，不能反查明文。
...
如果使用普通 `user key` 或 `admin key`，`account` 和 `user` 可以省略，因为服务端可以从 key 反查出来。Key 认证请求不要额外携带 `X-OpenViking-Account` / `X-OpenViking-User`；不带 password 的身份 header 只在 `trusted` 模式下使用。

Password 认证只在 `api_key` 模式下可用，并且必须同时传三个 header：
...
| PUT | `/api/v1/admin/accounts/{id}/users/{uid}/password` | ROOT, ADMIN | 设置用户密码 |
