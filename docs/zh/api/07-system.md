...
**profile 行为说明**:
- `profile` 仅对当前请求生效，请求结束后自动关闭，不会污染后续请求。
- 仅对 JSON 响应注入 `profile` 字段；纯文本、文件、流式响应不会被改写。
- 返回值是 `list[string]`，每个元素对应一行格式化后的 `pstats` 输出，便于浏览器直接以 JSON 数组查看。
- 路径会优先裁剪到包/模块起点，例如 `openviking/server/...`、`starlette/...`、`asyncio/...`，避免暴露绝对路径。
- 类似 `~:0(<method 'read' of 'builtins.RAGFSBindingClient' objects>)` 的条目表示内建方法或 C 扩展方法，不对应普通 `.py` 文件路径，这是 `cProfile/pstats` 的正常表现。
