...
- HTTP 端点接受已认证的 USER、ADMIN 或 ROOT；目标 URI 必须通过与普通写入相同的 `write` 鉴权。
- 个人区由 owner 规则控制；ACL 控制的共享资源要求 `editor` 或 `manager`。`viking://` 这类 account 级维护目标要求管理员身份。
- `api_key` 模式下，裸 root key 仍不能直接选择租户级数据。
...
列表表示 executor 支持的目标类型，不代表调用者自动拥有权限；每个请求仍按目标 URI 执行 `write` 鉴权。
