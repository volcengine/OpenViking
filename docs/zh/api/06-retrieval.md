...
| tags | List[str] | 否 | None | 显式检索标签，必须是严格的 `k=v` 格式（key/value 非空、只能有一个 `=`，key≤64、value≤128）。多个 tags 之间是 AND 关系，结果必须同时包含所有请求的标签 |
...
Tags 必须使用严格的 `k=v` 字符串：key 和 value 都非空，标签中只能有一个 `=`；允许其他字符，服务端会去除标签首尾空白并转小写，key 最长 64、value 最长 128 字符。传入多个 tags 时，`find()` 会要求全部命中；上面的例子只返回显式检索标签同时包含 `env=prod` 和 `team=search` 的上下文。
