...
| tags | List[str] | 否 | None | 显式检索标签字符串。规范化后的多个 tags 之间是 AND 关系，结果必须同时包含所有请求的标签 |
...
Tags 为字符串，标签层不限制字符、长度、空内容或 `=` 的数量。服务端会去除每个标签的首尾空白并转小写。对于 `k=v` 标签，以第一个 `=` 分隔 key 和 value，同 key 保留最后一个值；不含 `=` 的标签会原样保留并单独去重。规范化后包含多个 tags 时，`find()` 会要求全部命中；上面的例子只返回显式检索标签同时包含 `env=prod` 和 `team=search` 的上下文。
