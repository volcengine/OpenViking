...
| memory_policy | object | 否 | None | 会话默认的记忆抽取策略。可选的 `self` 和 `peer` 开关控制写入目标；可选的顶层 `memory_types` 将抽取限制为指定的 enabled memory schema。未传或为 `null` 时允许所有 enabled memory schema。可在 commit 时覆盖；非法结构或未知 memory type 会以 `InvalidArgumentError` 拒绝。 |
...
| memory_policy | object | 否 | None | 控制本次 commit 的记忆抽取策略。可选的 `self` 和 `peer` 开关控制写入目标；可选的顶层 `memory_types` 将抽取限制为指定的 enabled memory schema。未传或为 `null` 时允许所有 enabled memory schema。commit 级的值会覆盖会话级策略；非法结构或未知 memory type 会以 `InvalidArgumentError` 拒绝。 |
