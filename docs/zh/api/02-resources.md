...
| 类型 | 资源名 | 说明 |
|------|--------|------|
| 单页 / 递归抓取 | `https://host/path` | 默认仅抓入口页；当 `args.depth > 0` 或显式传入 `args.max_pages` 时，沿同域链接 BFS 递归展开。每页用 trafilatura 抽成 Markdown，对 SPA 等静态 HTML 拿不到正文的站点自动降级到 Playwright 渲染。可选 `args`：`depth`、`max_pages`、`include_paths`、`exclude_paths`、`allow_external_links`。`include_paths`/`exclude_paths` 按**路径前缀**匹配（例如 `/docs/` 仅匹配以 `/docs/` 开头的路径，不会误命中 `/blog/docs-tips`）。|
