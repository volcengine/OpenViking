...
2. 解析目标 URI
3. 按 `parse_mode` 调用对应 Parser，或将源文件按原始结构直接写入临时资源树
4. 构建目录树并写入 AGFS
...
| parse_mode | `default` \| `no_parse` | 否 | `default` | 入库策略。`default` 保持现有 Parser 解析和拆分行为；`no_parse` 不调用文件格式 Parser，不拆分文件，保留原文件名、字节和相对路径后继续语义处理与向量化 |
...
- `parse_mode=no_parse` 对单文件和目录中的所有文件使用同一规则：不做 Parser 解析或章节拆分，并保留原始字节。目录仍遵循 `.gitignore` 和上述筛选参数，且未知扩展名也可入库。该模式要求保留目录结构，与 `preserve_structure=false` 同时使用会被拒绝。
- `no_parse` 只改变文件 staging 策略，不关闭后续语义处理和向量化。PDF 和未知二进制文件不会在该模式下额外执行文本提取；若没有可用摘要，现有向量化器可能跳过对应叶子节点。
