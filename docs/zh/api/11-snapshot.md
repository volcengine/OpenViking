...
响应包含 `path`、解析后的 `from_commit` 和 `to_commit`、`change_type`（`added`、`deleted`、`modified` 或 `unchanged`）以及 `diff_text`。参与对比的单侧文件上限为 10 MiB，生成的 diff 上限为 20 MiB；超限时返回 `RESOURCE_EXHAUSTED`，不会返回被截断的 diff。
