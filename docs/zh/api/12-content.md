...
设置用于检索过滤的显式标签字符串。标签不允许包含英文逗号 `,`；本接口会丢弃含该字符的标签并记录警告日志。标签层不再限制其他字符、长度、空内容或 `=` 的数量。服务端会去除每个标签的首尾空白并转小写。对于 `k=v` 标签，以第一个 `=` 分隔 key 和 value，同 key 保留最后一个值；不含 `=` 的标签会原样保留并单独去重。`replace` 替换已有标签，`append` 合并标签并覆盖同 key 的旧值；对目录设置 `recursive=true` 时会更新目录下的文件。
