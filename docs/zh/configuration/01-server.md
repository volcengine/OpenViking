...
| `mcp_max_request_body_size_bytes` | 正整数 | `4194304` | MCP Streamable HTTP 请求体的最大字节数 |
...
OpenViking 对所有 MCP Streamable HTTP 方法的请求体应用 `mcp_max_request_body_size_bytes`。默认的 4 MiB 限制是 MCP SDK v2 迁移新增的限制；此前的 MCP SDK 在此处不限制请求体大小。超限请求会在 JSON 解析或工具执行前返回 HTTP 413。请保留客户端实际需要的最小限制，仅为可信工作负载提高此值。反向代理可能设置更低的实际限制。

对于大型资源或技能文件，请使用 `add_resource` 或 `add_skill` 返回的上传流程。`write` 工具没有上传回退路径。可信客户端如需内联发送超过 4 MiB 的内容，必须同时提高此限制和代理限制，或将内容拆分为较小的 `write` 调用：先使用 `create`/`replace`，再使用 `append`。
