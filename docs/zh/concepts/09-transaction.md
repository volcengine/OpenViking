...
OpenViking 通过**路径锁**保护核心写操作（`rm`、`mv`、`add_resource`、`session.commit`）的一致性，并通过持久化 **QueueFS** 任务恢复异步处理。
...
4. **异步任务持久化**：`session.commit` Phase 2 写入 SQLite QueueFS，进程重启后继续处理
...
`session.commit` 的 Phase 2 以任务写入 SQLite QueueFS。worker 成功处理后确认任务；进程中断时，未确认任务保留在队列中，服务重启后会重新消费。

```
SessionCommitMsg -> SQLite QueueFS -> SessionCommitProcessor
```

恢复语义为 at-least-once：任务可能重新执行，archive 的 `.done` 和 `.failed.json` 是终态标记。
...
| 问题 | 方案 |
|------|------|
| Phase 2 运行时进程退出，archive 永远缺少 `.done` | Phase 1 先持锁归档，再将 Phase 2 写入 SQLite QueueFS，重启后恢复消费 |

LLM 调用耗时不可控（5s~60s+），不能放在持锁操作内。设计拆为两个阶段：

```
Phase 1 — 归档（路径锁保护）：
  1. 将待归档消息写入 history/archive_N/messages.jsonl
  2. 将保留的消息尾部写回 session/messages.jsonl
  3. 更新 session meta
  4. 将 SessionCommitMsg 写入 SQLite QueueFS

Phase 2 — 记忆提取 + 写入（QueueFS worker）：
  1. 从 archive 重新加载消息
  2. 生成工作记忆摘要并提取长期/执行记忆（LLM）
  3. 写 relations，更新 active_count
  4. 合并 session meta
  5. 最后写 `.done`，并确认 QueueFS 任务
```
...
| 崩溃时间点 | 状态 | 恢复动作 |
|-----------|------|---------|
| Phase 1 写 archive 失败 | 尚未入队 | 恢复内存消息并返回错误，不启动 Phase 2 |
| Phase 1 已入队、worker 尚未处理 | QueueFS 中存在未确认任务 | worker 拉取并执行 Phase 2 |
| Phase 2 记忆提取/写入中途 | QueueFS 任务未确认 | 服务重启后从 archive 重新执行 |
| Phase 2 已写 `.done` | QueueFS 任务可确认 | 重复消费时读取 `.done` 并直接完成 |
...
异步任务由 QueueFS 在 worker 启动后恢复消费：

| 场景 | 恢复方式 |
|------|---------|
| session_memory 提取中途崩溃 | SQLite QueueFS 保留未确认任务，从 archive 重新执行 Phase 2 |
...
| session.commit Phase 2 崩溃 | QueueFS SQLite 持久化 + 从 archive 重做 | worker 重启后 |
