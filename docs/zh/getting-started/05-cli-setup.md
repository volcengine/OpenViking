...
- 可能需要 API Key 或 password 认证。
- 只有 root key 的配置和 password 认证配置需要 `--account` 和 `--user`。
...
- 一个可访问的 OpenViking 目标：
  ...
  - 自定义 OpenViking 服务端。
- 如果目标需要鉴权，需要准备 API Key 或 password。

API Key 和 password 都是敏感凭证。手动配置 API Key 时优先通过 `ov config` 的交互式输入框输入。Password 认证在 `ovcli.conf` 中配置。只有当你明确相信当前渠道时，才把凭证提供给 Agent。Agent 不能把凭证写进 shell 命令、日志、长期记忆或原始配置输出。
...
- Password：使用 `ovcli.conf` 中的 `account`、`user` 和 `password` 完成 password 认证。如果同时配置 `api_key` 和 `password`，API Key 优先生效。
...
- Password 可能允许访问你的 OpenViking 数据。
- 手动配置时，优先使用 `ov config` 的交互式输入框。
- 如果是 password 认证的 CLI 配置，请在 `ovcli.conf` 中管理凭证，并避免打印原始配置文件。
- Agent 辅助配置时，只有通过你明确相信的渠道提供 API Key。
- Agent 应通过 stdin 传入 API Key。只有当 API Key 环境变量已经存在于当前 shell 中时，才使用环境变量。
- 不要把 API Key 或 password 直接写进可能被 shell history 保存的命令。
