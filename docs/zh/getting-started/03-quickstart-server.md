...
远端 URL 可以直接放在 `path` 里。本地文件需要先调用 `POST /api/v1/resources/temp_upload` 上传，再用返回的 `temp_file_id` 调目标 API。`temp_upload` 默认使用本地临时存储；只有在你显式需要分布式共享临时上传时，才传 `upload_mode=shared`。裸 HTTP 如果导入本地目录，需要先把目录打成 `.zip` 再上传。
