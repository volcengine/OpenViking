...
启用子代理时，Compile 会读取每个来源一次，在任务内存中按约 **30,000 个字符**组织输入批次。小文件可以合批，长文件可以跨批；优先在章节、段落或完整 Markdown 表格行之间划分，并为后续段补充所在章节标题、文档 frontmatter 和表头。重复上下文也计入字符预算，另有每批最多 10 个不同来源的限制。
...
Resource Wiki 提交时，代码会根据 YAML `sources` 补充正文来源链接，兼容 `resource` 和 `path` 字段，并保留已有引用。导航根据本次提交和目标目录中保留的有效 Wiki 页面补齐，保留模型写的介绍，并创建缺失的祖先目录 `index.md`。这些处理不改变已有文件名。
