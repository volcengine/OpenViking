...
| **Cursor** | [Cursor 记忆插件](./12-cursor.md) — 一个插件内封装生命周期 Hook、MCP 工具、Rules 与 Skills |
...
Plugin 与 Hook/MCP 描述的是不同层级：Plugin 是可安装的封装与分发单元，内部可以包含 Hook、MCP Server、Rules 和 Skills。

- **Plugin / Extension**：负责封装和分发。例如 Cursor Plugin 内部包含 Hook、MCP Server、Rules 与 Skills。
- **生命周期 Hook**：由运行时事件自动触发召回与捕获，模型不需要主动调用工具。
- **MCP Server**：向模型暴露显式工具。通用 MCP 客户端只有这一层，不具备自动生命周期行为。
