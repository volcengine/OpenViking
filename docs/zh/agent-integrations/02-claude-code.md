...
在 `claude` 调用时注入 env vars 是同时覆盖 hooks 和 MCP 的唯一路径。用 shell function 包装（而不是全局 `export`）能把 API Key 限定在 `claude` 一个进程树内，不会泄漏到其他子进程——见 [步骤 1](#1-用-shell-function-在调用-claude-时注入-env) 的安全说明。
...
| 现象                                          | 原因                                                          | 修复                                                                                          |
|----------------------------------------------|--------------------------------------------------------------|----------------------------------------------------------------------------------------------|
| 插件未激活                                    | 找不到 `ov.conf` / `ovcli.conf`                              | 创建一份（见 [快速开始](#快速开始)），或者设 `OPENVIKING_MEMORY_ENABLED=1` + URL/API_KEY env vars |
