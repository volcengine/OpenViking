...
需要个性化 statusline（隐藏段位、改色、与已有 statusline 组合、加自定义段位）时，参见 [`examples/claude-code-memory-plugin/docs/STATUSLINE.md`](../../examples/claude-code-memory-plugin/docs/STATUSLINE.md)。这份文档是写给 AI assistant 读的——最方便的方式是打开 Claude Code 把链接喂给它，让它读完文档再帮你按偏好调整。
