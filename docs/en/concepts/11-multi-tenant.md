...
In `api_key` mode, configuring `server.root_api_key` enables key-based account and user management:
...
- `resources` includes account-shared resources by default and uses effective ACLs when account-level `acl.enabled` is on and grants are configured
...
- `user`: connect to an `api_key` Server using a User/Admin `api_key`; the key determines data identity.
- `root`: use a trusted Server connection to assert account/user identity. This key cannot access tenant data on an `api_key` Server.
- A Bot started from the same configuration can inherit the current Server connection; configure external Server connections explicitly.

Example for a fixed user identity:
...
Chat participants sharing one user key remain inside one OpenViking user boundary. `actor_peer_id` can restrict the peer collection view, but does not make that user's own memories or shared resources private to each participant. For separate user boundaries, a trusted entry point must supply distinct user connections or trusted identities; see [VikingBot Installation and Configuration](../guides/17-vikingbot.md).
...
| One gateway or bot service serves many end users | Per-user connections or trusted identity forwarding |
...
- VikingBot supports a fixed User/Admin key or trusted identity forwarding, according to the Server authentication mode
