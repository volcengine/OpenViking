...
Pass `actor_peer_id` to enable the peer-restricted view for retrieval/filesystem access. Session message capture can still use `peer_id` for per-message speaker attribution.
