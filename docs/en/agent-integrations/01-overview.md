...
| **AstrBot** (QQ / Telegram / Discord and other IMs) | [Other community plugins](./05-other-plugins.md#astrbot-plugin) — auto-capture group chats, semantic recall, configurable venue memory isolation |
...
- **Hooks-based plugins** (Claude Code, Codex, OpenClaw, AstrBot) drive recall and capture from runtime lifecycle events — every prompt, every turn, session start/end, compact, subagent spawn. The model doesn't need to "remember to recall."
