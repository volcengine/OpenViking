...
| dsh | `turn/end`: `pending_tokens ≥ 20000` (30s timeout), keep 10 | Teardown (see [§3.3.3](#_3-3-3-shutdown-method-×-harness-outcome-matrix)) | `compaction/start`: unconditional commit (keep 10; DSH appends the durable event before rewriting the transcript, so it covers the same boundary as the Claude Code PreCompact hook) |
...
| dsh | No takeover. The plugin commits unconditionally at the durable `compaction/start` event, before the host rewrites the transcript; injection rides on a pre-step user message and shrinks alongside the host's compaction, and the profile is not re-sent. | — | — |
