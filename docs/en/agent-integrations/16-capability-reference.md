...
This page compares tools, automatic recall, session writes, and exit behavior across agent hosts (harnesses). Capture writes conversation messages to a session; commit archives them and triggers memory extraction. A successful write does not mean long-term memory has been generated. Use the table below to find a behavior, then follow the integration page for setup.
...
| harness | how it plugs in | auto-recall | recall carries session_id | digest (client)* | profile injection | takes over host compaction | offline compensation | statusline |
|---|---|---|---|---|---|---|---|---|
...
| codex / trae-cli | 6 hooks + MCP proxy + skill | ✅ | ✅ | ✅ local `codex exec` (on by default) | ✅ (10000) + `<available-skills>` (1200) | ❌ | ✅ transcript + cursor | ❌ |
...
Tool definitions come from the server through `tools/list`. The proxy reads connection settings from the local `~/.openviking/ovcli.conf`, then connects to discover tools. After a server update, reconnect or refresh the tool list. Counts on this page describe the current repository implementation; the connected server determines the actual list.
...
| 15 | `forget` | Permanently deletes a URI (irreversible) | `recursive=False` by default; type boundaries in [§3.5](#_3-5-type-boundaries-for-writes-and-deletes) (`:1791`) |
...
- **Identity middleware** (`:149-233`): Shares `resolve_identity` with REST endpoints. Identity depends on the authentication mode: API key mode resolves the account and user from the key, trusted mode accepts identity headers from a trusted gateway, and dev mode uses the default identity. Actor peer is resolved separately and cannot override authentication. See [Authentication](../guides/04-authentication.md).
...
- **Server-side auto-commit is disabled by default unless an explicit or deployment-default policy enables it**:
  ...
  - Current memory plugins do not explicitly send an `auto_commit_policy`. Set it through the Python SDK’s `create_session` / `update_session_config`, the HTTP API, or the **ov CLI** (`ov session new --auto-commit-policy-json` / `--no-auto-commit`, `ov session config set`).
...
| ovcli.conf `plugin` section (shared scalars, overridden by a `plugin.<harness>` object) | claude-code / codex / cursor / trae / trae-cn / zcode / opencode / dsh / pi | A per-harness key takes either spelling — `claude_code` or `claude-code`, `trae_cn` or `trae-cn`. CLI edits and renames preserve extra top-level fields such as `plugin`. Switching uses fields explicitly supplied by the incoming profile and retains omitted extra fields from the active file. |
...
- **Actual timeout values**:
  ...
  - **opencode**: 30s. **dsh**: 10s configured HTTP default, raised to at least 15s for context expansion (`dsh` blocks the pre-step). **pi**: 15s.
...
Local compressor details:

- **claude-code**: Probes for a local compressor with `claude --version` (result cached for 7 days). If found, it runs `claude -p --model sonnet --effort low --strict-mcp-config` with a 30s timeout, skips inputs under 1500 characters (`recallCompressMinInputChars`), caches per digest, and snaps cited URIs back to valid hit URIs by edit distance, dropping bullets it cannot repair. A failed subprocess falls back to the uncompressed block.
- **codex**: Picks a model from `~/.codex/models_cache.json` (`gpt-5.3-codex-spark` first, then `gpt-5.6-luna` with low effort; cached for 7 days) and runs `codex --sandbox read-only --ask-for-approval never exec --ephemeral --ignore-user-config --skip-git-repo-check --output-last-message <tmp> -`, with a default timeout of `recallTimeoutMs - 10s` (110s). A runtime failure disables local compression for the rest of the session until the next `SessionStart` re-probes. Output is normalized and capped at 4000 characters; when compression is off or fails, a deterministic fallback digest is used.
...
- **Write-path detachment** (`async-writer.mjs`, enabled by default for `cc`/`codex`/`zcode` on `Stop`): The execution flow is drain stdin → spawn detached worker → approve → write payload → unref. *(Note: If spawning fails, approval has not yet occurred, ensuring the synchronous fallback executes exactly once).* The detached worker forms its own process group and can continue after the terminal exits. It must already have started; network errors, system termination, or server failures can still leave the write incomplete. **Side effect:** Once detachment is active, `Stop` no longer prints the `appended N turn(s)` notice (to restore this, set `OPENVIKING_WRITE_PATH_ASYNC=0`).
...
Legend: **C** = the exit event attempts to trigger a commit; **C\*** = additional conditions apply; **—** = this event does not trigger a commit; **n/a** = not applicable. A trigger does not guarantee completed archiving or memory extraction: network errors, host exit budgets, or process termination can interrupt the request. Successfully written messages remain in the server’s live area until a later commit. This table assumes server auto-commit is disabled; configured policies add the [server behavior](#_2-3-server-side-session-and-commit-semantics).
...
| pi (takeover default) | **—** (supported graceful shutdown paths await `session_shutdown`, but the handler persists local takeover state and does not commit) | **—** | **—** | **—** | **—** | The next run accumulating 30000, or a manual `/viking commit` |
...
| ov CLI | n/a (one-shot command) | n/a | n/a | n/a | n/a | No pending queue; inspect the task or target content before retrying a failed write |
| ingest | **C** (`finally _flush_all`) | **C** (SIGINT → stop) | **C** | **—** (no SIGHUP handler) | **—** | Recovery through persisted cursors: `needs_commit` is persisted in the cursor store, and the subsequent run's reconciliation completes the commit |
...
1. Normal exit attempts a commit in claude-code, codex (≥0.145), opencode (≥1.15.11), dsh, pi (takeover off), and hermes. Check the conditions and recovery column; an exit event is not confirmation that a commit completed.
...
| codex / trae-cli | Transcript + persisted cursor | New captures do not enter the pending queue. The cursor advances only for successfully sent messages; later capture or the SessionStart sweep resends the remaining tail. SessionStart also replays existing shared pending entries. |
...
| ingest | SQLite cursor store + single-instance lock | The intent is persisted before appending. After a crash, a reconciliation process checks the server's message count to determine if the batch landed, then decides whether to resend messages or retry the commit |

### 3.3.5 subagent session comparison

| harness | handling |
|---|---|
| claude-code | Separate subagent sessions: SubagentStart derives a separate `cc-<sid>__subagent-<agent_id>` session, and SubagentStop reads the subagent transcript, pushes it, commits unconditionally, and clears the state |
...
| codex / trae-cli | All hooks catch exceptions → noop | context-face 6h + compressor runtime_failed (until the next startup) | Capture retries use the transcript/cursor; SessionStart also replays existing pending entries | No |
...
- **Capability highlights**: The rollout file `~/.zcode/cli/rollout/model-io-<sid>.jsonl` serves as the source of truth for increments (a `lastTurnId` diff backfills any missed Stop events). Stop detaches by default. An already-started worker can continue after the terminal receives Ctrl+C, but network errors or process termination can still interrupt the write.
...
- **Integration docs**: [DSH Plugin](./17-dsh.md)
...
- **Highlights**: Resource ingestion: `viking_add_resource` supports HTTP, Git, SSH, `file://`, temporary uploads of local files, and zip-packing local directories for upload (automatically skipping symlinks and out-of-tree files). The `viking_remember` tool writes memory files directly, bypassing session commits or extractions. A local server can be initialized on demand; if the configured local endpoint is unreachable, it is automatically launched via `subprocess.Popen openviking-server`. It supports retries with an injected trusted identity. Normal exit, Ctrl+C, SIGTERM, and SIGHUP attempt to drain the queue and commit; an incomplete drain skips the commit, and the exit watchdog can interrupt slow requests ([§3.3.3](#_3-3-3-shutdown-method-×-harness-outcome-matrix)).
...
- **Highlights**: Provides command-line entry points for `auto_commit_policy` (`ov session new --auto-commit-policy-json`, `ov session config set`). It also offers multi-profile management, admin tools, privacy controls, snapshot management, and TUI capabilities unavailable in any plugin (see [§5.3](#_5-3-capabilities-only-the-cli-has)).
...
`ov` (internally identified as `openviking` in clap) is a Rust-based HTTP client. Most data operations call the server’s REST API. The CLI also packages local uploads, validates configuration, generates keys, renders output, and manages profiles. It is a standalone tool rather than a harness integration—meaning it does not handle host events, automatic recall, or compaction takeover. This chapter details its complete command surface for using OV directly (either manually or via scripts) and highlights additional CLI entry points (such as the TUI, multi-profile management, admin tools, `--sudo`, privacy controls, and snapshots).
...
**Skills**: `skills add` (supports local paths, Git, and GitHub tree URLs; use `-s` to select or `*` for all; prompts for interactive confirmation) | `skills list/find/show/update/remove` | `skills validate` (local validation without a server request)

**Sessions/memory**: `session new` (`--auto-commit-policy-json` and `--no-auto-commit` are mutually exclusive; the SDK and HTTP API also support setting the policy) | `session list/get/delete` | `session get-session-context` (`--token-budget` defaults to 128000) | `session get-session-archive` | `session add-message(s)` | `session config set` (modifies mutable configurations) | `session commit`
...
- **Multi-profile**: The active profile is `~/.openviking/ovcli.conf`; named profiles use `ovcli.conf.<name>`. Edits and renames preserve extra top-level fields such as `plugin`. Switching replaces connection settings. An incoming `plugin` section takes precedence; if it is absent, the active file’s existing `plugin` section is retained.
...
## 5.3 Additional CLI capabilities {#_5-3-capabilities-only-the-cli-has}

Compared with memory plugins, the CLI/TUI also provides the following entry points. Server-side operations can also be called through their HTTP APIs or SDK methods: multi-profile switching and the configuration wizard, the complete `admin` account and user management suite, root operations via `--sudo`, `privacy` policy CRUD and versioning, data movement commands (`snapshot`/`backup`/`restore`/`export`/`import`), index rebuilds via `reindex`, root key generation via `system crypto init-key`, interactive browsing with `ov tui`, VikingBot-dependent features like `ov chat` and `ov compile`, and `ov session config set` for explicitly defining the `auto_commit_policy` (the SDK and HTTP API can also set this policy).
...
MCP clients that support Streamable HTTP can connect to the server’s `/mcp` endpoint. This example is for clients using `mcpServers`, `url`, and `headers`; other clients use different fields and formats, as documented in [MCP Clients](./06-mcp-clients.md). The connection discovers the tools provided by the server ([§2.1](#_2-1-server-side-mcp-tool-surface)).
...
Use a user or admin API key; the server resolves the account and user from that key. `X-OpenViking-Account` and `X-OpenViking-User` belong to trusted identity mode and cannot override an ordinary key’s identity. Add `X-OpenViking-Actor-Peer` when workspace peer context is needed; see [Authentication](../guides/04-authentication.md) for visibility rules. For stdio-only clients, use the [Agent Plugins portable proxy](./15-agent-plugins.md) to bridge to Streamable HTTP. Direct MCP connections do not include automatic recall, capture, or session commits. Calling `remember` creates and commits a one-shot session.
...
If you need the full spectrum of automation—recall, capture, commit, and pending state management—there is no need to build it from scratch. Choose the implementation that matches the host. The portable package supplies MCP and skills, without automatic hooks:

- **`examples/memory-plugin-shared/lib/`** (Node): This provides a complete set of core modules. These include `recall-core` (three-tier degradation recall), `profile-inject`, `capture-utils` (message normalization and injection-echo protection), `pending-queue` (offline replay), `batch-send`, `mcp-proxy-core` (stdio↔HTTP proxy), `session-model` (session ID derivation), and `credentials`. To build a lightweight integration harness, map host lifecycle events to these modules and validate ordering, retries, timeouts, and exit behavior. For example, `agent-hook-runtime.mjs` is a ready-made, all-in-one runtime shared by Cursor, Trae, and zcode. Its stdin event parsing provides a starting point for another host adapter.
...
|---|---|---|---|---|---|
| **A. [Open WebUI](./08-community-plugins.md)** | Standalone FastAPI OpenAPI tool server | 7 local OpenAPI routes (`ov_search`/`ov_recall_memories`/`ov_add_memory`/`ov_list_memories`/`ov_read_resource`/`ov_add_resource`/`ov_session_status`), no deletion tool | No session concept | Transport: bare `httpx`, no retries or negative caching; `/health` only echoes the config and does not probe OpenViking. | Inactive until the process is explicitly started. |
| **B. [LangChain/LangGraph](./07-langchain-langgraph.md)** | Python SDK adapter layer (retriever/tools/store/middleware/recorder) | `create_openviking_tools()` provides 12 StructuredTools (`viking_forget` is not in the agent profile by default) | `thread_id`/`session_id` come from the caller; `CommitPolicy` defaults to `never` | Retry behavior: read-only methods automatically retry once, writes never retry (to prevent duplicates), and partial successes raise a structured exception that can be retried as a slice. | Requires explicit construction before taking effect. |
