...
| `temp_upload.default_mode` | str | Server-side default for `POST /api/v1/resources/temp_upload` when the client does not send `upload_mode`: `"local"` (per-instance disk, current single-node behavior) or `"shared"` (distributed shared store usable across replicas). Shared objects are stored in the current account's internal `viking://upload/<upload_id>/` directory and can be consumed repeatedly for `shared_ttl_seconds`; each shared upload removes expired directories from that account. | `"local"` |
...
| `temp_upload.shared_ttl_seconds` | int | Retention time shared by local and shared temporary uploads, in seconds. Each upload cleans files older than this for its mode; shared cleanup is limited to the current account's directories; must be greater than `0`. | `43200` (12 hours) |
