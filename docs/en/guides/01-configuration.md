...
| `temp_upload.default_mode` | str | Server-side default for `POST /api/v1/resources/temp_upload` when the client does not send `upload_mode`: `"local"` (per-instance disk, current single-node behavior) or `"shared"` (distributed shared store usable across replicas). New shared uploads are stored as the current account's internal `viking://upload/<upload_id>.content` and `viking://upload/<upload_id>.meta` objects, and can be consumed repeatedly for `ttl_seconds`. | `"local"` |
...
| `temp_upload.ttl_seconds` | int | Retention time shared by local and shared temporary uploads, in seconds. Each upload cleans files older than this for its mode; shared cleanup is limited to the current account and uses one upload-root listing. A completed shared upload expires from its `.meta` modification time; an orphaned `.content` object expires from its own modification time. Set to `0` to disable automatic cleanup. | `43200` (12 hours) |
