...
User keys are generated by the Admin API. By default they use random secret
material; Admin API callers may also provide a `seed` to derive a predictable
key secret from `sha256(user_id + "\0" + seed)`. The server still verifies the
resulting key against the stored key or key hash.
