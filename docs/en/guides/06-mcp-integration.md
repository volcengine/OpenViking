...
> Feishu/Lark imports with `args.feishu_access_token` are one-time only. They cannot be combined with `watch_interval > 0` because OpenViking does not store or refresh user tokens.
