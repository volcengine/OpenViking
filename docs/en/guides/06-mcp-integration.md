...
| `read` | Read one or more `viking://` URIs. PNG, JPEG, GIF, and WebP files are returned as native MCP image content for vision-capable clients; other files keep the text response format | `uris` (single string or array) |
