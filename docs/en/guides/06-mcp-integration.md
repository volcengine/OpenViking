...
| `read` | Read one or more `viking://` URIs. PNG, JPEG, GIF, and WebP return native MCP image content; WAV, MP3, FLAC, OGG, and M4A return native audio content. MCP has no standard video content block, so video reads return a download hint | `uris` (single string or array) |
