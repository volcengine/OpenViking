...
| `read` | Read one or more `viking://` URIs. PNG, JPEG, GIF, and WebP return native MCP image content; WAV, MP3, FLAC, OGG, and M4A return native audio content. MCP has no standard video block, so video reads tell the agent to retry in download mode, which exports the original bytes as an embedded resource | `uris` (single string or array), `mode` (`content` default or `download`) |
