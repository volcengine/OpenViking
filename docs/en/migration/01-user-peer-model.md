...
| `find` / `search` with HTTP SDK `agent_id` | Supported. Empty-target search includes user memory plus the selected actor peer view; old agent data is searched only by true legacy requests that send `X-OpenViking-Agent`. |
| `remember` / `ov add-memory` with `actor_peer_id` | The default remains user memory. Pass an exact `viking://user/<user_id>/peers/<peer_id>/memories` target with `target_uri` / `--target-uri` to write peer memory. |
