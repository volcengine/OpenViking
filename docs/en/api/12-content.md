...
A memory file's indexed `abstract` comes only from the actual summary stored in `MEMORY_FIELDS.summary`, capped at 8,000 UTF-8 bytes. The body remains embedding input under the existing rules; it is never used as an abstract fallback. Reindex also does not reuse an old vector record's `abstract`, which may contain the previous body.

- `vectors_only` reuses the persisted summary. If it is missing, the abstract stays empty while the body is vectorized, without a model call to generate a summary. Records with empty abstracts keep their vector scores and skip abstract-based reranking.
- For a single memory file or a recursive memory directory, `semantic_and_vectors` reuses existing summaries and generates missing ones into `MEMORY_FIELDS.summary` before rebuilding vectors. Summary generation or persistence failures stop the subsequent vector rebuild. Directory `recursive=false` continues to update only directory semantics and L0/L1 vectors.
- Normal memory writes use the same summary source. Body changes without a replacement summary invalidate the previous summary; a later semantic rebuild can regenerate it. Missing source files are no longer reindexed from an old vector record's abstract alone.
