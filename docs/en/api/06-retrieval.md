...
Use `find` for semantic retrieval without session context, `search` for optional session-aware retrieval, `grep` for content patterns, and `glob` for file paths.
...
```
Query → Optional session planning (search only) → Vector retrieval → Optional directory traversal and rerank → Results
```

1. **Intent Analysis** (search only): Understand query intent, expand queries
2. **Vector Search**: Find candidates using embeddings
3. **Rerank** (when configured): Re-score candidates using the rerank model
...
`find()` uses the retriever's QUICK mode: one vector search within the allowed scope, then score filtering, URI deduplication, and ranking. It does not expand directories, call the reranker, or apply hotness scoring. With no query or image, a supplied filter selects records directly and returns `score=0`.

**Processing Pipeline**:
1. Convert query text to vector
2. Perform global vector search within specified target URI
3. Apply the score threshold, deduplicate URIs, and sort candidates
4. Return up to `limit` matches
...
class MatchedContext:
    ...
    score: float                     # Ranking score; not a probability
...
`search()` can plan queries from session context. Text queries use directory traversal and reranking when a reranker is configured; otherwise they use the QUICK retrieval path. Image queries always use QUICK mode. It calls the LLM only when `retrieval.enable_intent=true` and the session has a summary or messages. Without a session, with an empty session, or with intent analysis disabled, it uses the original query. Image queries skip session planning.
...
4. Retrieve each planned query using the configured retrieval path
...
| image_url | str | No | None | Image query as a `data:image/...;base64,...`, `http(s)://`, or `viking://` URI. Requires a multimodal embedding model |
| target_uri | str \| List[str] | No | "" | Limit search to specific URI prefix |
| session_id | str | No | None | Session ID for context-aware search (HTTP and SDKs) |
| context_type | str \| List[str] | No | None | Limit results to one or more `ContextType` values: `memory`, `resource`, or `skill` |
| tags | List[str] | No | None | Explicit retrieval tags in strict `k=v` form. Multiple tags are combined with AND; a result must contain every requested tag |
...
The L0–L3 labels below name assembly stages, not the stored L0/L1/L2 content layers.
