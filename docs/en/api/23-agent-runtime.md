...
Resource compilation uses two stages. Source children produce drafts; `wait_subagents(wait_all=true)` returns their complete inventory, file sizes in bytes, summaries and failures in one collection. After resolving failures, the parent groups canonical topics, aliases and differently named versions from this global inventory, reading short headings or metadata only for ambiguous entries. All drafts for a topic stay with one merge child, and batches are balanced by total draft size. `spawn(draft_paths=[...])` attaches complete submitted draft contents to the child’s initial input, avoiding repeated file-reading rounds. Attachments are bounded to one quarter of the smaller initial-prompt/context budget and are never truncated: split independent topics when necessary, or use bounded reads for a single oversized topic. Each output page has one owner, and the parent generates all navigation. The parent assembles results, checks formatting, coverage and path conflicts, removes invalid staged files and dangling navigation links, reports exclusions, and submits while preserving existing target files. Cancellation stops running and queued children; salvage excludes child draft directories.

`bot.agents.subagent_max_concurrency` limits parallel children per Compile task (default `8`, excluding the parent). Running tasks, queued tasks, and uncollected results together are capped at twice this limit; queued work starts automatically. Both stages reuse this capacity and queue, with further assignments guided by `wait_subagents` and retries limited to failed groups. Grouping, Skill checks, and exclusions are agent actions guided by the prompt and selected Skill; submission separately validates paths, sizes, and OKF formatting.
