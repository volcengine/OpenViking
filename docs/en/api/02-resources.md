...
- Local Markdown images that resolve within the allowed import roots and pass the parser's image checks are copied beside the parsed Markdown file. Both `![alt](./img/a.png)` references and HTML `<img src="./img/a.png">` tags are rewritten to `viking://...` URIs after persistence. Remote URLs, existing `viking://` URIs, data URIs, code spans/blocks, and unresolved or rejected local images keep their original references.
