...
| Type | Description |
|------|-------------|
| Feishu/Lark | URL-based, supports doc/docx, wiki, sheets, bitable. A Wiki node is imported as one document by default; set `args.feishu_wiki_scope="subtree"` to import its descendants while preserving the Wiki hierarchy. By default uses app credentials from FEISHU_APP_ID and FEISHU_APP_SECRET; user-token imports can pass `args.feishu_access_token`, and user-token watches also pass `args.feishu_refresh_token` plus an optional `args.feishu_app_id` / `args.feishu_app_secret` pair |
...
- A `/wiki/{node_token}` URL keeps the existing single-document behavior by default. Set `args.feishu_wiki_scope="subtree"` to import that node and every descendant as one directory resource. An explicitly requested subtree remains a directory resource even when the selected node currently has no children. A `/wiki/settings/{space_id}` URL always imports the whole space as a subtree; `/wiki/settings` without a space ID is invalid. Supplying `feishu_wiki_scope` for a non-Wiki source, or `node` scope for a space URL, is rejected before ingestion starts.
- Wiki subtree imports preserve each node placement as a directory: the node body is stored as `{title}.md` in that directory and child nodes are nested below it. Placement identity uses `node_token`, so two Wiki nodes that reference the same `obj_token` remain separate. Duplicate sibling titles receive a stable node-token suffix.
- Subtree discovery fails instead of silently producing a partial tree when child listing fails, pagination repeats or omits a required page token, a `node_token` repeats, depth exceeds 20 parent-child edges, or the import exceeds 500 nodes. These fixed limits are fail-closed; select a smaller subtree if either limit is exceeded.
- Feishu API calls retry transient transport failures, HTTP 408/429/5xx responses, and known rate-limit responses up to three total attempts. Numeric `Retry-After` is honored with a bounded delay; other retries use bounded exponential backoff with jitter. Permission, authentication, validation, and not-found failures are not retried.
- Individual inaccessible or unsupported node bodies are reported in `meta.failed_files` while accessible nodes are imported; `strict=true` makes those body failures fatal. If every discovered body fails, the whole import fails instead of returning an empty successful resource. Subtree metadata includes `feishu_wiki_node_count`, `feishu_wiki_materialized_count`, and `feishu_wiki_skipped_count`.
...
# Import a Feishu Wiki node and all descendants, preserving hierarchy
curl -X POST http://localhost:1933/api/v1/resources \
  -H "Content-Type: application/json" \
  -H "X-API-Key: your-key" \
  -d '{
    "path": "https://example.feishu.cn/wiki/node_token",
    "args": {
