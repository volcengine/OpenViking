...
| source_headers | object | No | - | HTTP headers attached while fetching an HTTP(S) `path`. For example, a private TOS object may use `{"X-Tos-Signature": "..."}`. Headers are used for both the type-detection HEAD and content-download GET; the source is fetched as an in-request snapshot, and headers are not written to resource metadata or later queue jobs. |
