...
Claude Code and Codex share this installer. Every step is idempotent, so it is safe to rerun. No shell wrapper is needed — the plugin ships a stdio MCP proxy that reads `~/.openviking/ovcli.conf` at runtime.
