...
- 幂等标识派生为稳定任务 ID，参数摘要保存在任务记录中但不通过公共 API 回显；同标识的并发创建在当前实例内串行。保障建立在现有单 TaskTracker 写入者模型上，多写入者部署仍需存储级 CAS。
- 幂等恢复与任务记录共用保留周期。当前 tracker 对完成/取消任务保留 24 小时，对失败任务保留 7 天；此功能不新增永久历史。页面对超过 24 小时的持久提交标识停止自动重发。
