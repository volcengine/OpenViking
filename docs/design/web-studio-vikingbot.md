...
支持更新凭证、重新检测、暂停与恢复；暂停停止接收新任务，已有任务按明确的排空策略结束，保留历史。删除连接需要二次确认，并说明会删除本地收发记录，不会删除飞书应用或飞书中的消息。凭证更新失败保留原连接，不能破坏其他渠道。

默认开启“仅 @机器人时回复”。需要接收未 @ 消息时，可调整“群聊回复方式”，同时补齐飞书权限并发布；普通群和话题回复的具体规则见文末“扫码任务与回复配置”。
