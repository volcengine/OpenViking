...
- `agent_evolution_enabled=false`：从本次 commit 的有效类型中移除 `cases`、`trajectories` 和 `experiences`。
...
- Working Memory 摘要。
- profile、preferences、events 等非 Agent 记忆提取。
- 已有 case、trajectory 和 experience 文件的 list、find、search、read。
...
4. 开关关闭时，从 session policy 中移除 `cases`、`trajectories` 和 `experiences`。
...
- 存量 case、trajectory 和 experience 文件保持原 URI 和检索可见性。
