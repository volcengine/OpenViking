...
MCP 与 HTTP 共享同一套纯 ranking/formatting 实现，仅 I/O 层不同。
...
{
  "type": "viking",
  "uri": "viking://resources/owner/repo/sklearn/utils/_pprint.py",
  "relative_path": "sklearn/utils/_pprint.py"
}
```

Viking source 没有本地 checkout 映射时，验证建议不会生成可执行本地命令，只返回 target 和 setup note。
...
{
  "type": "local",
  "path": "/abs/repo/path/sklearn/utils/_pprint.py",
  "relative_path": "sklearn/utils/_pprint.py"
}
```
...
```json
{
  ...
  "query": {
    "text": "Bug in print_changed_only repr for vector-valued estimator params",
    "failing_tests": [
...
```json
{
  ...
  "symbols": [],
  "reasons": [
    "exact identifiers: print_changed_only",
...
{
  "kind": "static",
  "command": "python3 -m py_compile sklearn/utils/_pprint.py",
  "cwd": "/abs/repo/path",
  "targets": [
    {
      "type": "local",
...
{
  "kind": "narrow_tests",
  "command": "python3 -m pytest sklearn/utils/tests/test_pprint.py",
  "cwd": "/abs/repo/path",
  "targets": [],
...
Python 命令统一使用 `python3 -m ...`，比 `python -m ...` 更适合常见 Linux agent harness。
...
- staged action 输出以 `OpenViking staged action:` 开头，突出 PATCH FIRST、edit line、message shape line、replacement sketch 和 immediate verification。
