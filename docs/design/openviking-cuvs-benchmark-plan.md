...
低精度 GPU 路线分开评估：先增加 cuVS float16 dataset/query，与 float32 ground
truth 比较 Recall@K、延迟和显存；随后再评估 CAGRA int8 或 VPQ。native 的逐向量
scale int8 不能直接映射到 cuVS brute-force，若要求数值兼容，需要自定义 int8
distance/top-k 或候选召回后按 scale rerank，不能用简单 cast 代替。
