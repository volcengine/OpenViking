...
ResourceService.add_resource                  对外入口，只负责顶层分流
        ...
        |                                         +--> AddResourceMsg 队列
        |
        +-- 飞书 URL && enable_feishu_url && 有凭证
        |       |
        |       +-- wait=false + user token --> 先提交 Understanding
        |       |                               --> 队列只保存 response_id
        |       |
        |       +-- 其他情况 ------------------> Worker / 当前请求直达 Understanding
        |
...
当 `parser_api.enable=true`、`parser_api.enable_feishu_url=true`，并且本次请求有 `args.feishu_access_token` 或服务端已配置飞书应用凭证时，原始飞书 URL 会在 Accessor 前直达 Understanding：

```text
飞书 URL + user/tenant token
    |
    v
Understanding API --> 结果 ZIP（Markdown + 图片）
    |
    v
ParseResult --> TreeBuilder
```

`enable_feishu_url` 默认是 `false`。配置未开启或直达链没有可用飞书凭证时会回退 `FeishuAccessor`，而不是误送外部解析；如果 Accessor 本身也拿不到 user/app 凭证，最终仍会返回明确的飞书鉴权错误。因此 Accessor 的 sheets/base 读取和素材下载能力仍是稳定默认路径；开启直达后，内容覆盖范围取决于 Understanding 服务返回的 artifact。
...
飞书直达的异步路径也冻结 `parser_backend="understanding"`。显式 user token 不能写入 QueueFS，因此生产端先调用 `submit_url`，队列只持久化 `understanding_response_id`，Worker 从该 response 继续轮询，不重复提交也不重新判断配置。使用应用凭证时，Worker 获取 tenant token 后提交。未显式指定资源名的飞书 artifact 会从 ZIP 单一根目录恢复真实标题，目标 URI 因而延迟到解析完成后确定。
