...
Understanding 是统一的高优先级解析链。原始来源能被直接提交时先进入 Understanding；需要识别本地类型时才由 Accessor 物化，再根据 `resolved_extension` 进入同一条 Understanding 链。当 `parser_api.enable=true`、`parser_api.enable_feishu_url=true`，并且本次请求有 `args.feishu_access_token` 或服务端已配置飞书应用凭证时，原始飞书 URL 可直接提交，因此会在 Accessor 前命中：
