...
// Derive GitHub repo URL from socialLinks
const repoLink = (theme.value.socialLinks as Array<{ icon: string; link: string }> | undefined)
  ?.find(l => l.icon === 'github')?.link ?? 'https://github.com/volcengine/OpenViking'

const githubFileUrl = `${repoLink}/blob/main/docs/${page.value.relativePath}`
...
async function rawMarkdown(): Promise<string> {
  return frontmatter.value._rawMarkdown ?? ''
}

async function copyToClipboard() {
  const md = await rawMarkdown()
  if (!md) return
...
async function handleItem(item: typeof items[number]) {
  if (item.copyFirst) await copyToClipboard()
...
const isDoc = page.value.relativePath !== 'index.md'
</script>
