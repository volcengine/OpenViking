...
- OpenClaw：OpenViking 插件当前暴露 `ov_search` 并同步 `openclaw.plugin.json`；`memory_recall` 继续作为召回语义保留，不和 search 混成一个通用工具；原 Agent 可见 `ov_import` 已拆成 `add_resource` / `add_skill`。
