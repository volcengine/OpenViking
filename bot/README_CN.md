...
- ov_server：OpenViking Server 配置。
  ...
  - 若不使用本地启动的OpenViking Server，可在此配置url和对应的root user的API Key
    ...
    - exp_write_tools: 可选，触发经验记忆注入的工具名列表（自演化 agent memory 循环，详见 #2007）。默认 `["write_file", "edit_file"]`。注入仅在 OpenViking server 启用 `memory.experimental_memory_switch` 时生效，否则此列表无作用。
