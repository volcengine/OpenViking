...
- **实验性 FUSE 挂载**：将 `viking://` URI 映射到本地文件系统
...
## 🔬 OpenViking 挂载模块（实验功能）

Vikingbot 包含一个实验性的 FUSE 文件系统挂载模块，可以将 OpenViking 资源映射到本地文件系统。

### 功能特性

- **URI 映射**：`viking://{scope}/{path}` → 本地文件
- **多作用域支持**：
  - `resources` - OpenViking 资源
  - `session` - 会话数据
  - `user` - 用户数据
  - `agent` - Agent 数据
- **与会话集成**：每个会话独立挂载
...
查看 `vikingbot/openviking_mount/README.md` 了解更多详情。
