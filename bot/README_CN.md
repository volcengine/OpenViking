...
- ov_server：OpenViking Server 配置。
  ...
  - 若不使用本地启动的OpenViking Server，可在此配置url和对应的root user的API Key
    ...
    - api_key_type: 可选 `root` 或 `user`，默认 `root`（旧 fanout 行为：bot 使用 root key 调用 OpenViking，并自动注册多用户）。设置为 `user` 切换到 user-key 流程：bot 仍用 root key 引导每个用户，但后续调用使用该用户的 API Key。
    - exp_write_tools: 可选，触发经验记忆注入的工具名列表（自演化 agent memory 循环，详见 `agent` 配置）。默认 `["write_file", "edit_file"]`。
