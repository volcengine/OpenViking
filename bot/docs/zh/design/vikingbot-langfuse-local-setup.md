...
This guide is for local development and validation. It uses the repository's bundled Docker Compose deployment and credentials generated uniquely on first launch.
...
The script creates `bot/deploy/docker/langfuse/.env` with mode `0600` on first
...
- Admin password: value of `LANGFUSE_INIT_USER_PASSWORD` in `.env`
- Public key: value of `LANGFUSE_INIT_PROJECT_PUBLIC_KEY` in `.env`
- Secret key: value of `LANGFUSE_INIT_PROJECT_SECRET_KEY` in `.env`
...
```json
{
  "bot": {
    "langfuse": {
      "enabled": true,
      "secret_key": "<LANGFUSE_INIT_PROJECT_SECRET_KEY from .env>",
      "public_key": "<LANGFUSE_INIT_PROJECT_PUBLIC_KEY from .env>",
...
2. Password: the `LANGFUSE_INIT_USER_PASSWORD` value in
   `bot/deploy/docker/langfuse/.env`
