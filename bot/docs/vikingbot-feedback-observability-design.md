...
- 还没有 `response_id`，一次最终回答无法被稳定引用和回查
- 还没有显式反馈入口，OpenAPI 也没有点赞/点踩/评分接口
- 还没有 `response_completed`、`feedback_submitted`、`response_outcome_evaluated` 这类结果事件
...
- Langfuse 当前写入的 generation metadata 仍较薄，尚未覆盖 `response_id`、`query_category`、`prompt_version`、`bot_version`、`outcome_label`
...
第一阶段实际上只必须新增一类核心事件：`response_completed`。

`feedback_submitted` 和 `response_outcome_evaluated` 应分别放在第二、第三阶段，因为它们都依赖 `response_id` 和结构化响应事实先稳定下来。
...
- 可直接在 Langfuse 中筛选的 `outcome_label`
...
2. 再补 `query_category`
3. 最后再补 `prompt_version`、`bot_version`、`final_outcome_label`
