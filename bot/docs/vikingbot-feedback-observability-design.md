...
- `response_completed` 与 `feedback_submitted` 已经进入实现范围；`response_outcome_evaluated` 仍然属于 Phase 3，当前未实现
...
- Langfuse 当前写入的 generation metadata 已覆盖 `response_id`，但仍未覆盖 `query_category`、`prompt_version`、`bot_version`、`outcome_label`
...
5. `response_outcome_evaluated` 仍然是 Phase 3 目标，本文档不应把它描述成当前已存在能力。
...
3. `response_outcome_evaluated` 仍应保留在第三阶段，因为它依赖更稳定的 session 后处理与隐式 outcome 规则。
