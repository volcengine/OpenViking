#!/usr/bin/env python3
"""Step 2 (Effectiveness): Build vector indexes for imported code repos.
...
Prerequisites:
  1. Run step1_add_resource.py to import repos (without indexing)
...
PROGRESS_FILE = os.path.expanduser("~/.openviking/data/benchmark/.effectiveness-reindex-progress")
...
def main():
    parser = argparse.ArgumentParser(
        description="Step 2 (Effectiveness): Build vector indexes via openviking-server"
    )
    parser.add_argument(
        "--source",
    ...
    print("=" * 80)
    print("Step 2 (Effectiveness): Build Vector Indexes (via openviking-server)")
    print("=" * 80)
...
def _print_summary(results: list[dict], skipped_count: int, all_dirs: list[str]) -> None:
    ...
    if total_done >= total and failed_count == 0:
        ...
        print("Next step: run step3_quality.py to evaluate retrieval quality")
    else:
        print(
