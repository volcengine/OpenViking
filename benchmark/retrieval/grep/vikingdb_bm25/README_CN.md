...
| 2 | `step2_reindex.py` | 通过 openviking-server 异步构建索引（并发=2，轮询） |
| 3 | `step3_quality.py` | SDK grep 与本地正则 ground truth 对比 |
...
# 可选参数：--concurrency N  （默认：2）

# 步骤 3：编辑 step3_quality.py 中的 KEYWORDS 列表，然后运行
...
- **Effectiveness（效果测试）** 使用真实代码，对照 ground truth 计算精确的召回率/精确率
- **Performance（性能测试）** 使用合成数据，根据已知注入概率计算近似召回率
