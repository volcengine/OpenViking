...
The adaptation stays inside this benchmark harness. VikingBot runtime does not receive
deprecated agent identity, peer identity, or a synthetic user override; it only consumes the
configured authenticated user.
Per-domain isolation is achieved by provisioning a separate user-key config before the run.
