...
3. Review the returned candidates. Each candidate includes its `case_name`. An `exact_case` result contains each linked experience's full `content`; those experiences are already loaded inline. A `semantic` result contains an experience `uri` and a short `situation` snippet describing applicability and exclusions.
4. For an `exact_case` result, use each non-empty inline `content` directly and do not call `read_experience` again for those entries. For a `semantic` result, treat `situation` as a filter only, not as the full experience.
5. **Read by default.** For each semantic-search experience, call `read_experience` unless its `situation` explicitly excludes the current task family, object type, policy branch, or requested operation. Do not skip reading merely because the experience may apply later rather than at the opening request.
...
8. After each semantic search, read every candidate that is not explicitly excluded. If multiple experiences pass this gate, read each applicable experience. Exact-case experience content needs no additional read.
...
- `search_experience(situation, task_signature=None, limit=2)`: when Runtime Case context supplies `task_signature`, first reads that exact Case and returns the full content of all linked experiences in stable URI order; this content is already loaded inline. Only a missing exact Case falls back to Situation search. Without `task_signature`, searches OpenViking `memories/cases` under the current user using the declarative current Situation. Semantic-search experience entries contain only `uri` and a `situation` snippet from the experience's `## Situation` section.
