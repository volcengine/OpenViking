"""
Evaluate LoCoMo QA via OpenClaw + modelstudio-memory-for-openclaw plugin.

For each sample, updates ~/.openclaw/openclaw.json to set userId = sample_id,
restarts the OpenClaw gateway, then sends questions via /v1/responses API.

Prerequisites:
  - Conversations already ingested via ingest.py (userId = sample_id)
  - OpenClaw installed with modelstudio-memory-for-openclaw plugin configured
...
PLUGIN_KEY = "modelstudio-memory-for-openclaw"
...
def _update_plugin_user(sample_id: str) -> None:
    """
    Rewrite ~/.openclaw/openclaw.json so that the plugin uses sample_id as userId.
    ...
    plugin_entry = entries.setdefault(PLUGIN_KEY, {})
    plugin_entry["enabled"] = True
    plugin_entry.setdefault("config", {})["userId"] = sample_id
...
def _restart_gateway(base_url: str, startup_timeout: int = 30) -> None:
    """
    Kill and restart the OpenClaw gateway, wait until healthy.
    Must be called while holding _openclaw_config_lock.
    """
    try:
...
def parse_session_tokens(session_id: str) -> dict:
...
def send_to_openclaw(
    ...
    """Send a question to OpenClaw. Returns (response_text, usage, time_cost)."""
    if question_time:
    ...
    session_key = f"locomo-eval-{question_id}-{suffix}" if question_id else f"locomo-eval-{suffix}"
    ...
    if not remaining:
        ...
        for sample_id, qa_list in by_sample.items():
            ...
            with _openclaw_config_lock:
                _update_plugin_user(sample_id)
                _restart_gateway(args.openclaw_url)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate LoCoMo QA via OpenClaw + modelstudio-memory-for-openclaw plugin",
