...
def _sync_bot_identity(account_id: str, user_id: str, api_key: str) -> None:
    ...
    if changed:
        ...
        _log(
            f"已同步 bot.ov_server: api_key_type=user, account_id={account_id}, admin_user_id={user_id}"
        )
...
def _resolve_openviking_api_key() -> tuple[str, str]:
    ...
    _error(
        "未配置 OpenViking User API key，请设置 bot.ov_server.api_key 或 ovcli.conf.api_key 后重试。"
    )
    raise SystemExit(1)
...
def _ensure_server_and_user_key_ready(
    ...
    _log(
        f"OpenViking server 可用，User key 身份: account={account_id}, user={user_id}, role={role}。"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-env-file", required=True, help="File path to write ACCOUNT/OPENVIKING_URL exports"
    )
