...
| `~/.openviking/codex-plugin-state/<session_id>.json` | Per-session state: `ovSessionId` (`cx-<session_id>`, null once committed), `transcriptPath` (last rollout seen, used by the SessionStart sweep to catch up unsent turns), `capturedTurnCount`, `captureFormatVersion` (2; absent means the old cursor format and is migrated on the next readable rollout), `lastUpdatedAt`. |
