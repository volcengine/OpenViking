...
test("auto-recall prefers the server recall endpoint when available", async () => {
  ...
  try {
    ...
    assert.equal(requests[0].body.admission, undefined);
  } finally {
    await rm(stateDir, { recursive: true, force: true });
  }
});

test("auto-recall forwards optional admission policy to the recall endpoint", async () => {
  ...
  assert.deepEqual(result.requestBody.admission, {
    mode: "shadow",
    type_min_scores: { events: 0.5, entities: 0.55 },
    other_peer_score_delta: 0.08,
  });
});

test("auto-recall fails closed when enforce mode is unsupported by the server", async () => {
  ...
  try {
    ...
    assert.deepEqual(paths, ["/health", "/api/v1/search/recall"]);
  } finally {
    await rm(stateDir, { recursive: true, force: true });
  }
});
