...
Two knobs narrow which turns reach that list. `OPENVIKING_CAPTURE_TOOL_TRAFFIC` (default `1`) drops tool traffic outright when set to `0`: both the `[tool-call|result]` text and the structured `tool` parts go, and an entry that is nothing but a tool call or result stops being a turn at all. This is the bulk of a coding session — a tool result is reported verbatim and can carry a whole file — so an operator who wants conversation memory rather than a command log turns it off. `OPENVIKING_CAPTURE_ASSISTANT_FINAL_ONLY` (default `0`) keeps one assistant reply per user turn when set to `1`, namely the last one, so a reply the model rewrote a few times is stored once, in its final shape. A tool result normalizes to the `user` role, so it never ends a turn and cannot split one in two. Neither knob touches what is already stored, `captureFilters` still runs over whatever is left, and both are read from the environment, from `plugin.captureToolTraffic` / `plugin.captureAssistantFinalOnly`, or from the same keys under `plugin.codex` or the legacy `codex` block — see [Configuration](#configuration) for the resolution order.
