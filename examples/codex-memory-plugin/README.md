...
Rules run top to bottom and the first `d` that matches (or `k` that does not) ends the decision; text a substitution empties is not a drop, just too short to recall on. Filters run before `OPENVIKING_MIN_QUERY_LENGTH` and before the built-in ack / slash-command heuristics. A capture rule shapes what is sent, not what is already stored, and anything already in the pending queue carries the rules that were in effect when it was enqueued; adding a `d`/`k` rule mid-session also shortens the turn list the cursor counts, which reads as a transcript rewrite and replays from the last user turn. A rule that fails to compile is skipped, never fatal. Config loading warns on stderr with the filter environment-variable name, zero-based rule index and parse error, even when debug logging is off; this also applies to rules loaded from configuration files. `ov-memory-doctor` lists the active rules and reports the exact error for the ones it could not parse. Bare regexes such as `sk-[A-Za-z0-9-]+` are not rules: use `s/sk-[A-Za-z0-9-]+/[redacted]/g` to redact matches.
