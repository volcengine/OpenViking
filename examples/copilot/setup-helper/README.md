...
- GitHub Copilot CLI: installs `@openviking/copilot-cli-memory`, merges the `mcp.json` server entry, and can add the optional `copilot()` wrapper.
...
| `COPILOT_MCP_JSON` | OS-specific Copilot CLI `mcp.json` path |
