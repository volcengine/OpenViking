...
- `tools/pre-execute` blocks DSH filesystem tools from using `viking://` URIs in path arguments and blocks shell commands that would open one as a file (`cat viking://…`, a redirection target), pointing the model at the bridged `mcp__openviking__*` tools instead. A command that only carries the URI as data — an HTTP payload, the `ov` CLI's own argument, a grep pattern, a heredoc — is left alone.
