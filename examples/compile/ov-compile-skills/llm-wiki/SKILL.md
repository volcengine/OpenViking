...
正文直接解释页面主题，再按内容组织属性、参数、规则、步骤、例外或问答。保留数字、单位、适用条件、限制、时效、版本、生效日期和来源对应关系。合并重复事实及同义问题，不逐条扩写背景。不兼容的说法保留各自依据和适用范围，推断和未知明确标注。来源章节集中列出可读的原始引用；需要区分结论或冲突时就近补引用。
...
- H1 为知识库或目录的可读名称，随后用一句话说明用途。frontmatter 增加 `last_updated: YYYY-MM-DD`，正文末尾标注更新日期，与 `generated.at` 一致；未变更时保留原日期。
- 有子目录时，先用“导航”表列出目录名称、说明和对应 `index.md` 链接，再为每个子目录设独立章节，逐层展开。目录名称按配置的中文含义展示；无子目录时直接列出页面，可按业务主题分组。
- 每个索引列全其目录子树内的知识页，根索引可直接导航到全库每页。条目为 `[页面标题](相对路径.md) — 一句具体的查阅说明`，说明来自正文或 `description`。链接必须指向真实文件，不遗漏页面，不用代表条目、“等”或“20+ 页”代替完整链接。
