...
本地开发时，也可以在插件配置中设置 `"autoStartServer": true`。启用后，
插件只会在 `http://localhost:1933`、`http://127.0.0.1:1933` 等本地端点
不可用时尝试启动 `openviking-server`；远程端点仍需要由外部环境负责启动和运维。
...
`autoStartServer` 默认关闭，以保持显式管理 server 生命周期的现有行为。
只有当你希望插件在本地端点不可用时自动启动本地 OpenViking server，才需要将它设为 `true`。
