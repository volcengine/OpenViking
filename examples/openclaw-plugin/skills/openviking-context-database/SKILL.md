...
| `agent_prefix` | empty | Optional prefix for `X-OpenViking-Agent`; final value is `<prefix>_<OpenClaw ctx.agentId>`, sanitized. |
| `accountId` / `userId` | empty | Advanced tenant identity headers for root-key or trusted deployments. |
| `isolateUserScopeByAgent` | `false` | Canonical namespace policy for `viking://user/...`; must match server account policy. |
| `isolateAgentScopeByUser` | `false` | Canonical namespace policy for `viking://agent/...`; must match server account policy. |
...
```bash
openclaw openviking setup --base-url <URL> --api-key <KEY> --agent-prefix openclaw-prod --json
...
Semantic search over memories/resources/sessions.
...
| `resourceTypes` | No | Array of `resource`, `session`, `user`, `agent`; used only when `targetUri` is omitted. |
...
Import Agent Skills into `viking://agent/skills/...`.
...
| `resourceTypes` | No | Array/string containing `resource`, `session`, `user`, `agent`. |
