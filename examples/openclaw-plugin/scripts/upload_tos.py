...
def public_url(spec: BucketSpec, file_name: str, release_path: str | None = None, internal: bool = True) -> str:
...
def is_not_found_error(err: Exception) -> bool:
    status = getattr(err, "status_code", None) or getattr(err, "status", None) or getattr(err, "code", None)
...
def upload_one(client, spec: BucketSpec, release_path: str, path: pathlib.Path, dry_run: bool = False) -> None:
...
def stamp_bucket_installer(install_sh: pathlib.Path, spec: BucketSpec, release_path: str, output_dir: pathlib.Path) -> pathlib.Path:
    text = install_sh.read_text(encoding="utf-8")
    text = re.sub(r'^DEFAULT_TOS_BASE_URL=".*"$', f'DEFAULT_TOS_BASE_URL="{bucket_base_url(spec)}"', text, flags=re.MULTILINE)
    text = re.sub(
...
def stamp_bucket_manifest(manifest: pathlib.Path, spec: BucketSpec, release_path: str, stamped_install: pathlib.Path, output_dir: pathlib.Path) -> pathlib.Path:
    ...
    for artifact in data.get("artifacts", []):
        if artifact.get("name") == "install.sh":
            ...
            artifact["path"] = object_key(release_path, pathlib.PurePosixPath(artifact["path"]).name)
    stamped = output_dir / "manifest.json"
    stamped.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return stamped


def upload_bucket_installer(client, spec: BucketSpec, release_path: str, install_sh: pathlib.Path, output_dir: pathlib.Path, dry_run: bool = False) -> pathlib.Path:
...
def upload_files(client, spec: BucketSpec, artifacts: list[pathlib.Path], paths: list[str], dry_run: bool = False) -> None:
    ...
    for release_path in paths:
        with tempfile.TemporaryDirectory(prefix="openviking-installer-") as tmp:
            ...
            stamped_install = upload_bucket_installer(client, spec, release_path, install_sh, tmp_dir, dry_run=dry_run) if install_sh else None
            stamped_manifest = stamp_bucket_manifest(manifest, spec, release_path, stamped_install, tmp_dir) if manifest and stamped_install else None
...
def parse_args():
    parser = argparse.ArgumentParser(description="Upload OpenViking install.sh, openviking.tgz, and manifest.json to all TOS buckets.")
    ...
    parser.add_argument("--release-dir", default="", help="Date release directory, for example 2026.6.3. Defaults to today's yyyy.m.d.")
    parser.add_argument(
        "--publish-latest",
        action="store_true",
...
def main() -> int:
    ...
    for spec in BUCKET_SPECS:
        ...
        for release_path in paths:
            print(f"  {release_path}/install.sh:    {public_url(spec, 'install.sh', release_path=release_path)}")
            print(f"  {release_path}/openviking.tgz: {public_url(spec, 'openviking.tgz', release_path=release_path)}")
            print(f"  {release_path}/manifest.json:  {public_url(spec, 'manifest.json', release_path=release_path)}")

    if not args.publish_latest:
        print("latest/ was not updated. Re-run with --publish-latest only after the dated release is stable.")
