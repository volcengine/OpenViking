...
OPENVIKING_AGENT_PREFIX="${OPENVIKING_AGENT_PREFIX:-}"
...
configure_openviking_service() {
  ...
  if [ "$DRY_RUN" -eq 0 ]; then
    ...
    write_env_line "OPENVIKING_AGENT_PREFIX" "$OPENVIKING_AGENT_PREFIX"
  ...
  [ -z "$OPENVIKING_AGENT_PREFIX" ] || set -- "$@" --agent-prefix "$OPENVIKING_AGENT_PREFIX"
