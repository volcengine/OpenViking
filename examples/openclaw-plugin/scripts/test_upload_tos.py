...
def test_object_keys_and_public_urls():
    ...
    assert module.public_url(spec, "install.sh") == (
...
def test_release_dir_can_be_specified_and_is_not_hardcoded():
    ...
    assert module.release_paths(custom_release_dir, publish_latest=True) == [custom_release_dir, "latest"]
...
def test_upload_sets_public_read_acl_and_content_types(tmp_path):
    ...
    class FakeClient:
        ...
        def put_object_acl(self, **kwargs):
            calls.append({"acl_bucket": kwargs["bucket"], "acl_key": kwargs["key"], "acl": kwargs["acl"]})
