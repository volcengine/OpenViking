...
function inferRecallResourceType(uri: string): RecallResourceType | undefined {
...
function createTraceId(source: string): string {
...
const contextEnginePlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    const registerOpenVikingTool = (toolOrFactory: unknown, opts: { name: string }) => {
      if (!enabledToolNames.has(opts.name)) {
        api.logger.debug?.(`openviking: tool ${opts.name} disabled by config`);
        return;
      }
      api.registerTool(toolOrFactory as any, opts);
    };
      ...
      const entries = await Promise.all(result.entries.map(async (entry) => {
        const selected = await Promise.all(entry.selected.map(async (item) => {
          try {
            ...
            return { ...item, contentPreview: previewText(text, cfg.recallMaxContentChars) };
          } catch (err) {
    ...
    const registerRecallTraceRoutes = (ctx?: unknown): boolean => {
      ...
      const toQueryObject = (request?: { query?: Record<string, unknown>; params?: Record<string, unknown>; url?: string }) => {
      ...
      const handle = async (request?: { query?: Record<string, unknown>; params?: Record<string, unknown>; url?: string }) => {
    ...
    const readOpenVikingContent = async (input: OVReadInput, agentId?: string) => {
      const uri = input.uri.trim();
      if (!uri) {
        throw new Error("uri is required");
      }
      if (!uri.startsWith("viking://")) {
        throw new Error("ov_read only accepts OpenViking viking:// URIs, not local file paths or openviking:// display aliases");
      }
      if (uri.endsWith("...") || uri.includes("…")) {
        throw new Error(
          "ov_read received a truncated display URI. Use the exact full viking:// URI from ov_search details/results; do not shorten it with ... or ….",
        );
      }
      const client = await getClient();
      ...
      return {
        content: [{ type: "text" as const, text: `OpenViking content for ${uri}\n\n${text}` }],
        details: {
          action: "read",
          uri,
        },
    ...
    const searchOpenViking = async (input: OVSearchInput, agentId?: string, traceCtx?: PluginSessionRouting) => {
    ...
    registerOpenVikingTool(
      (ctx: ToolContext) => ({
        ...
        description:
          ...
          "Returned viking:// URIs are OpenViking virtual URIs, not local file paths; use ov_read to read them.",
        parameters: Type.Object({
    ...
    registerOpenVikingTool(
      (ctx: ToolContext) => ({
        name: "ov_read",
        label: "Read Content (OpenViking)",
        description:
          "Read full content for an exact OpenViking viking:// URI returned by ov_search or recall traces. " +
    ...
    api.registerCommand?.({
      ...
      handler: async (ctx: PluginCommandContext) => {
        try {
          ...
          const result = await searchOpenViking(input, session.agentId, session);
          return { text: result.content[0]!.text, details: result.details };
        } catch (err) {
      ...
      (ctx: ToolContext) => ({
        ...
        async execute(_toolCallId: string, params: Record<string, unknown>) {
          ...
          try {
            ...
            const traceResults: RecallTraceResult[] = (result.matches ?? []).slice(0, cfg.traceRecallMaxResultsPerSearch).map((match) => ({
              uri: match.uri,
