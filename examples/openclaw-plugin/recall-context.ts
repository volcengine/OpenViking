...
type RecallPrecheckResult = { ok: true } | { ok: false; reason: string };
    ...
    return await withTimeout(
      (async () => {
        ...
        const findPromises = [
          client.find(
            queryText,
            { targetUri: "viking://user/memories", limit: candidateLimit, scoreThreshold: 0 },
            agentId,
          ),
          client.find(
            queryText,
            { targetUri: "viking://agent/memories", limit: candidateLimit, scoreThreshold: 0 },
            agentId,
          ),
        ];
        if (cfg.recallResources) {
          findPromises.push(
            client.find(
              queryText,
              { targetUri: "viking://resources", limit: candidateLimit, scoreThreshold: 0 },
              agentId,
            ),
          );
        }

        const settled = await Promise.allSettled(findPromises);
        ...
        const { lines: memoryLines, estimatedTokens } =
