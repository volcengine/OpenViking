...
- **导入与检索**：Agent 可导入 resources 和 Agent Skills，检索它们，并在召回中选择 `resource`、`session`、`user`、`agent` 等目标。
...
openclaw openviking setup \
  ...
  --agent-prefix "openclaw-prod" \
  --json
openclaw gateway restart
openclaw openviking status --json
```

该流程通过 ClawHub 安装插件，并完成 `openclaw openviking setup`、Gateway 重启和状态验证。TOS release 包安装、上传、下载和回滚流程不在本示例范围内。
...
| 工具 | 用途 |
|------|------|
| `memory_recall` | 在 `user`、`agent`、`session`、`resource` 目标中显式语义召回 |
| `memory_store` | 将重要文本写入 session，并立即触发记忆抽取 |
...
| `add_skill` | 将 `SKILL.md`、skill 目录、原始 skill 内容或 MCP tool dict 导入 `viking://agent/skills/...` |
...
- **存储位置**：所有数据存储在你的 OpenViking 服务上，命名空间包括 `viking://user/*`、`viking://agent/*`、`viking://session/*`、`viking://resources/*` 等。
- **API Key**：通过 `X-API-Key` header 发送，不会被日志记录或转发。
- **多租户隔离**：支持 `accountId`、`userId`、`agent_prefix`，以及 canonical namespace policy 开关（`isolateUserScopeByAgent`、`isolateAgentScopeByUser`）。
...
- `memory_recall`：在 memory/resource/session 目标上显式语义召回
- `memory_store`：把文本写入 OpenViking session 并触发阻塞 commit/抽取
