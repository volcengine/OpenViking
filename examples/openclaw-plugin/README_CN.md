...
- Hook 层：接管 `session_start`、`session_end`、`agent_end`、`before_reset`；`before_prompt_build` 仅保留为旧版召回兼容路径
...
默认自动召回在 `assemble()` 中运行，并把 `<relevant-memories>` 块注入到组装后的 messages 中。这样即使 OpenClaw 的 context-engine transform 路径没有消费 hook `prependContext` 或 `systemPromptAddition`，模型仍然能看到召回内容。

只有需要兼容旧版 `before_prompt_build` 行为时，才把 `recallPath` 设置为 `hook`。流程是：
...
- 当前版本里，自动 recall 的主入口是 `assemble()`；`before_prompt_build` 只是旧版兼容选项。
