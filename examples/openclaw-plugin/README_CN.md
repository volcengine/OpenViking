...
当本轮最新一条 user 消息匹配显式记忆意图（例如 "remember this"、"don't forget"，或中文 "记住" / "写入长期记忆" / "保存到长期记忆" 等），`afterTurn()` 会在 `pending_tokens` 还未达到 `commitTokenThreshold` 时也触发 `commit(wait=false)`。匹配规则以正则数组形式内联在 `services/context-lifecycle-service.ts` 顶部，后续如需可在跟进 PR 中改造为可配置。该行为默认开启，可通过 `intentCommitEnabled: false` 关闭。为避免高频"记住！"造成 commit 风暴，强制 commit 在进程级被限速为 30 秒最多一次。
