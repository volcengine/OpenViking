...
![Context engine assemble 自动召回流程](./images/openclaw-plugin-recall-flow.png)

自动召回只通过 context engine 的 `assemble()` 执行。`before_prompt_build` 只记录 session 路由信息，不再注入召回上下文，因此当前插件要求 OpenClaw 宿主支持带当前 prompt 的 context-engine assemble。

主 `assemble()` 召回流程是：

1. 优先使用当前 `prompt` 作为召回 query；只有在旧式 message-only 调用形态下，才回退到 `messages` 中最后一条用户文本。
2. 基于当前 `sessionId/sessionKey` 解析本轮的 agent 路由。
3. 先做一次快速可用性检查，避免在 OpenViking 不可用时把上下文组装链路拖死。
4. 并行检索 `viking://user/memories` 和 `viking://agent/memories`。
5. 在插件侧做去重、阈值筛选、重排和 token budget 裁剪。
6. 把最终记忆块以 `<relevant-memories>` 形式注入当前 turn 可见的通道：当 `prompt` 单独传入时使用 `systemPromptAddition`，只有旧式 message-only 调用才会写入最后一条 user message。
...
- 当前版本里，自动 recall 只由 context-engine `assemble()` 处理；`before_prompt_build` 不再注入记忆上下文。
