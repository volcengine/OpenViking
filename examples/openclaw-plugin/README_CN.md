...
OpenViking 应负责持久上下文，而不是承接所有外部数据源。X/Twitter 研究场景可搭配 [TweetClaw](https://github.com/Xquik-dev/tweetclaw)，也就是 `@xquik/tweetclaw` OpenClaw 插件，用于搜索推文、搜索推文回复、导出关注者、查询用户、监控推文、发送 webhook，以及在审批后发推或回复。
...
```text
请用 TweetClaw 搜索最近关于 "OpenViking context database" 的推文和回复。
把来源查询、日期、反复出现的需求、代表性 tweet ID 或链接、下一步行动，
整理成一条简洁的 OpenViking 记忆。
```

X/Twitter 凭据应留在本地 OpenClaw 配置中。不要把 API Key、签名密钥、原始 cookie 或完整时间线写入 OpenViking 记忆；只保存摘要结论和可供后续 Agent 召回的稳定引用。
