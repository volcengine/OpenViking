...
2. 验证插件是否按照服务侧命名空间策略，将 `viking://user/memories` 与 `viking://user/memories` 正确展开为 canonical URI。
3. 验证 `senderId` / `requesterSenderId` 是否能够稳定映射为会话消息的 `peer_id`。
4. 验证 `afterTurn` 与 `memory_store` 两条写会话路径的身份语义是否一致。
5. 验证多租户场景下用户空间、共享资源与 peer/session metadata 的召回行为是否符合预期。
...
- 远端 `19950`：用于 `api_key` 模式与用户空间验证
- 远端 `19960`：用于 `trusted` 模式与 `peer_id` 验证
...
针对 `peer_id` 透传行为，使用真实机器人私聊/群聊场景复核：

- 私聊验证真实 sender 到 `peer_id` 的稳定映射
- 群聊验证不同成员写入不同 `peer_id`
- 工具调用验证 `memory_store` 路径是否能通过 `requesterSenderId` 写出 `peer_id`
...
- user-scoped memory：`viking://user/<user_id>/memories`
- user-scoped skills：`viking://user/<user_id>/skills`
- shared resources：`viking://resources`

### 3.3 身份透传

- `afterTurn.runtimeContext.senderId -> peer_id`
- `memory_store.requesterSenderId -> peer_id`
- assistant message 不传 `peer_id`
...
1. 插件侧 user namespace、显式租户 header 与 `peer_id` 透传能力已完成联调验证。
2. `afterTurn` 路径可通过 `runtimeContext.senderId` 正确写出 `peer_id`。
3. `memory_store` 路径在工具上下文中无法直接读取 `senderId`，但可稳定通过 `requesterSenderId` 写出 `peer_id`。
4. 在真实机器人私聊与群聊场景中，已确认不同用户会被映射为不同的 `peer_id`，且 assistant message 保持 `peer_id:null`。
5. 当前 OpenViking 插件链路已满足多租户、命名空间与 sender 身份透传的测试目标。

## 6. 建议

1. 若后续服务端对 `api_key + USER` 的 `peer_id` 语义收紧，应补充一条显式兼容说明或服务端协同约束。
2. 如需继续扩大矩阵，可追加：
   - 更多账号/用户组合
   - 不同群聊成员的稳定性回归
