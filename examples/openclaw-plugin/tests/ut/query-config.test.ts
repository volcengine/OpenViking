...
describe("normalizeRuntimeQueryParams", () => {
  it("clamps values and normalizes resourceTypes", () => {
    const normalized = normalizeRuntimeQueryParams({
      ...
      maxInjectedChars: 10,
      ovSearchLimit: 999,
      resourceTypes: "resource,session,user",
    });
...
describe("RuntimeQueryConfigStore", () => {
  it("merges default, static, claw, session, and request layers with field sources", async () => {
    ...
    await store.set("claw", { agentId: "agent-a" }, { recallLimit: 8, scoreThreshold: 0.25, resourceTypes: ["resource"] });
    await store.set("session", { agentId: "agent-a", ovSessionId: "ov-session" }, { recallLimit: 3 });

    const effective = await store.getEffective(
      { agentId: "agent-a", ovSessionId: "ov-session" },
      { scoreThreshold: 0.05 },
    );
  ...
  it("preserves lower-priority explicit candidateLimit when a higher-priority layer changes only recallLimit", async () => {
    ...
    await store.set("claw", { agentId: "agent-a" }, { candidateLimit: 80 });
    await store.set("session", { agentId: "agent-a", sessionId: "s1" }, { recallLimit: 3 });

    const effective = await store.getEffective({ agentId: "agent-a", sessionId: "s1" });
  ...
  it("persists runtime config and reloads modified files", async () => {
    ...
    try {
      ...
      await store.load();
      await store.set("claw", { agentId: "agent-a" }, { recallLimit: 9 });

      const saved = JSON.parse(await readFile(path, "utf8"));
      expect(saved.claws["agent-a"].params.recallLimit).toBe(9);

      saved.claws["agent-a"].params.recallLimit = 4;
      await writeFile(path, JSON.stringify(saved), "utf8");
      await store.reloadIfChanged({ force: true });

      const effective = await store.getEffective({ agentId: "agent-a" });
  ...
  it("serializes writes behind an in-flight initial load", async () => {
    ...
    try {
      ...
      await writeFile(path, JSON.stringify({
        schemaVersion: "1.0",
        updatedAt: Date.now(),
        claws: { "agent-a": { params: { recallLimit: 9 }, updatedAt: Date.now() } },
        sessions: {},
      }), "utf8");
      const store = new RuntimeQueryConfigStore({ staticConfig: cfg, path });

      const loadPromise = store.load();
      const setPromise = store.set("claw", { agentId: "agent-a" }, { recallLimit: 2 });
  ...
  it("keeps the persistence queue usable after a transient write failure", async () => {
    ...
    try {
      ...
      await expect(store.set("claw", { agentId: "agent-a" }, { recallLimit: 2 })).rejects.toThrow();
      await rm(blockedParent, { force: true });

      await expect(store.set("claw", { agentId: "agent-a" }, { recallLimit: 3 })).resolves.toBeDefined();
      const saved = JSON.parse(await readFile(path, "utf8"));
      expect(saved.claws["agent-a"].params.recallLimit).toBe(3);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
  ...
  it("unsets one field and resets scoped runtime config", async () => {
    ...
    await store.set("session", { agentId: "agent-a", sessionId: "s1" }, { recallLimit: 2, scoreThreshold: 0.4 });
    expect((await store.getEffective({ agentId: "agent-a", sessionId: "s1" })).recallLimit).toBe(2);

    await store.unset("session", { agentId: "agent-a", sessionId: "s1" }, ["recallLimit"]);
    const afterUnset = await store.getEffective({ agentId: "agent-a", sessionId: "s1" });
    expect(afterUnset.recallLimit).toBe(6);
    expect(afterUnset.scoreThreshold).toBe(0.4);

    await store.reset("session", { agentId: "agent-a", sessionId: "s1" });
    const afterReset = await store.getEffective({ agentId: "agent-a", sessionId: "s1" });
