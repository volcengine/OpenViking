...
describe("plugin module seams", () => {
    ...
    const entry = {
      ...
      searches: [{
        ...
        results: [
          ...
          { uri: "viking://agent/skills/debugger", resourceType: "agent" as const, category: "skill", score: 0.8, abstractPreview: "Skill abstract", resultType: "skill" as const },
        ],
      }],
  ...
  it("creates OpenViking query runtime for search/read behavior", async () => {
    const find = vi.fn(async (_query: string, options: { targetUri?: string }) => {
      ...
      return {
        memories: [],
        resources: [],
        skills: [{ uri: "viking://agent/skills/debugger", overview: "Debugger overview", score: 0.8, category: "skill" }],
        total: 1,
      };
    });
    ...
    expect(find.mock.calls.map(([, options]) => options.targetUri)).toEqual(["viking://resources", "viking://agent/skills"]);
  ...
  it("creates OpenViking import runtime for command import behavior", async () => {
    ...
    const addSkill = vi.fn().mockResolvedValue({
      uri: "viking://agent/skills/debugger",
      name: "debugger",
    ...
    await expect(runtime.addSkillOpenViking({
      ...
      content: [{ type: "text", text: "Imported OpenViking skill (debugger). viking://agent/skills/debugger" }],
      details: {
  ...
  it("registers import tools through a dedicated plugin module", async () => {
    ...
    const addSkill = vi.fn().mockResolvedValue({ uri: "viking://agent/skills/demo", name: "demo" });
    const deps = {
      registerTool,
      getClient: async () => ({ addResource, addSkill }),
    ...
    expect(skillResult.content[0].text).toBe("Imported OpenViking skill (demo). viking://agent/skills/demo");
    expect(skillResult.details).toMatchObject({ action: "skill_imported", uri: "viking://agent/skills/demo" });
  });
