...
| --- | --- | --- |
| `resource` | `context_type=resource` | resource 知识库；默认 target URI 可为空，由服务端解析当前可检索资源范围。 |
| `user` | `context_type=memory` | 当前用户长期记忆。 |
| `agent` | `context_type=memory` + `peer_id` | 对应 peer/assistant 的长期记忆；不再依赖旧的 `viking://agent/...` 目录。 |
