...
- 本轮到底搜索了哪些范围：`resource`、`session`、`user`、`agent`？
    ...
    "entries": {
      "openviking": {
        "config": {
          ...
          "recallTargetTypes": ["user", "agent", "resource", "session"]
        }
      }
    }
  }
...
| 配置 | 默认/行为 | 说明 |
| --- | --- | --- |
| `recallTargetTypes` | 默认 `['user', 'agent']` | 允许值：`resource`、`session`、`user`、`agent`；空值回退默认集合。实现见 `config.ts:176`、`recall-trace.ts:113`。 |
...
| `session` | `viking://session/<ovSessionId>/history` | 当前 OpenClaw session 映射到的 OpenViking 会话历史；缺少 `ovSessionId` 时跳过，并在 trace search 中记录 `error: 'missing_session'`。 |
...
| `resourceTypes` | array | 本次 trace 覆盖的召回类型：`resource`、`session`、`user`、`agent`。 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `resourceType` | `resource` \| `session` \| `user` \| `agent` \| `archive` | 当前子搜索类型。 |
...
| `error` | string? | 子搜索失败或跳过原因，如 `missing_session`。 |
...
| `resourceTypes` | string[] 或逗号分隔 string | 无 | 按 trace 的 `resourceTypes` 过滤；允许 `resource`、`session`、`user`、`agent`。归一化见 `recall-trace.ts:113`。 |
...
- `searches[].error` 是否有 `missing_session` 或搜索失败。
...
| session 类型 trace 里有 `missing_session` | 当前上下文缺少可解析的 `ovSessionId` | 带 `sessionId/sessionKey` 查询，或确认调用场景有 OpenClaw session。搜索计划跳过逻辑见 `recall-trace.ts:156`。 |
