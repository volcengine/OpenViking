...
| 多租户/多 Agent 记忆串用 | 不同 session/agent 可能共用错误上下文 | 插件按 `sessionId/sessionKey/agentId/agent_prefix` 解析 `X-OpenViking-Agent`，并支持 account/user header 与 canonical namespace policy |
...
| `config.ts` | 插件配置 schema、默认值、环境变量解析、canonical namespace policy 配置 |
...
3. 创建 `OpenVikingClient`，注入 `baseUrl`、`apiKey`、`agent_prefix`、超时、租户与 namespace policy：`index.ts:625`。
...
Agent 路由由 `createSessionAgentResolver` 维护，优先从 session context 解析/记忆 agent，然后根据 `agent_prefix` 生成 `X-OpenViking-Agent`：`index.ts:470`。字符会经过 `sanitizeOpenVikingAgentIdHeader` 清洗，保证只包含 `[a-zA-Z0-9_-]`：`index.ts:226`。
...
4. 按 `recallTargetTypes` 并行搜索召回目标；默认搜索 `viking://user/memories`、`viking://agent/memories`，显式设为 `resource` 时只搜索 `viking://resources`；旧字段 `recallResources=true` 仅在未显式配置 `recallTargetTypes` 时把 `resource` 追加到默认集合。
...
| `add_skill` / `/add-skill` | 导入 `SKILL.md` 或 skill 目录 | `viking://agent/skills/...` |
...
| `X-OpenViking-Agent` | 当前 session 解析出的 agentId | 用于 agent scope 隔离 |
...
| API | 官方用途 | 当前插件映射 | 说明 |
| --- | --- | --- | --- |
| `GET /api/v1/fs/ls?uri=viking://agent/skills/` | 列 skill | `ov_search` 默认会搜 skills；未单独 list | 官方 `List Skills` 页面本质复用 `fs/ls` |
| `POST /api/v1/skills` | Add Skill / MCP tool conversion | `add_skill` | 与资源导入章节相同 |
| 读取 `viking://agent/skills/{name}/SKILL.md` | 读 skill 全文 | `ov_read` 或 `content/read` 手工读取 | 官方建议按 L0/L1/L2 逐级读取 |
...
认证方式：OpenViking HTTP 支持 `X-API-Key: <key>` 和 `Authorization: Bearer <key>`；插件固定使用 `X-API-Key`。如果服务端启用了多租户且当前 key 需要显式租户上下文，插件还会附加 `X-OpenViking-Account`、`X-OpenViking-User`、`X-OpenViking-Agent`。
...
| `viking://agent/memories` | 当前 agent 长期记忆别名 |
| `viking://resources` | account/resource 知识库 |
| `viking://agent/skills` | 当前 agent skill 空间 |
| `viking://session/{sessionId}/history` | session archive 历史 |
| `viking://session/{sessionId}/tool-results/{id}` | 外置工具结果 |

对于 `viking://user/...` 和 `viking://agent/...`，插件会按 canonical namespace policy 展开为带 user/agent id 的真实 URI：`client.ts:389`、`client.ts:403`。

关键注意：`isolateUserScopeByAgent` / `isolateAgentScopeByUser` 必须与服务端 account namespace policy 保持一致，否则会出现“写进 A 命名空间、搜 B 命名空间”的问题。
...
| `agent_prefix` | 空 | Agent 路由前缀；非空时形成 `<prefix>_<ctx.agentId>` |
| `isolateUserScopeByAgent` | `false` | user scope 是否按 agent 隔离 |
| `isolateAgentScopeByUser` | `false` | agent scope 是否按 user 隔离 |
...
| `agent_prefix` | Agent 路由、技能检索空间 | 空 | 是 | 间接支持 | 可通过配置值写 `${ENV}` | 非空时拼成 `<prefix>_<ctx.agentId>`，会影响 `viking://agent/...` 的检索命名空间：`config.ts:74`、`config.ts:268` |
| `isolateUserScopeByAgent` | user memory 检索空间 | `false` | 是 | 是 | `OPENVIKING_ISOLATE_USER_SCOPE_BY_AGENT` | 决定 `viking://user/memories` 是否展开到 `viking://user/<user>/agent/<agent>/...`：`config.ts:226` |
| `isolateAgentScopeByUser` | agent memory / skill 检索空间 | `false` | 是 | 是 | `OPENVIKING_ISOLATE_AGENT_SCOPE_BY_USER` | 决定 `viking://agent/...` 是否展开到 `viking://agent/<agent>/user/<user>/...`：`config.ts:230` |
| `agentScopeMode` | 旧版 scope 兼容 | `agent` | 是 | 是 | `OPENVIKING_AGENT_SCOPE_MODE` | 已废弃兼容项；仅旧部署迁移时使用，建议改用上面两个布尔开关：`config.ts:221` |
...
| `OPENVIKING_AGENT_SCOPE_MODE` | `agentScopeMode` | 旧版 scope 兼容模式 |
| `OPENVIKING_ISOLATE_USER_SCOPE_BY_AGENT` | `isolateUserScopeByAgent` | user scope 是否按 agent 隔离 |
| `OPENVIKING_ISOLATE_AGENT_SCOPE_BY_USER` | `isolateAgentScopeByUser` | agent scope 是否按 user 隔离 |
...
| 同样的 query 在不同 agent / user 命中不一致 | `accountId`、`userId`、`agent_prefix`、`isolate*` | 路由到的 canonical namespace 不一致 |
...
日志会打印 `X-OpenViking-Agent`、account/user header 是否设置、target_uri、query preview、session commit 等信息，但不会打印 apiKey。
...
# 查 agent skills
ov_search(query="用户问题关键词", uri="viking://agent/skills", limit=10)
```
...
export OPENVIKING_AGENT="<X-OpenViking-Agent-from-log>"
...
headers=(
  ...
  -H "X-OpenViking-Agent: $OPENVIKING_AGENT"
)
...
| 导入 skill | `POST /api/v1/skills` | `add_skill` / `/add-skill` | 将 skill 写入 `viking://agent/skills` |
...
- 如果未显式配置 `recallTargetTypes`，自动召回默认只查 `viking://user/memories` 和 `viking://agent/memories`，不会把 `viking://resources` 文档自动注入；resource-only 用 `recallTargetTypes=["resource"]`，默认记忆 + resources 用 `recallResources=true` 或 `recallTargetTypes=["user","agent","resource"]`。
...
| 不同 Agent 记忆串用 | `logFindRequests` 中的 `X-OpenViking-Agent` | `agent_prefix` 或 session agent 解析不符合预期 |
...
| 测试文件 | 覆盖重点 |
| --- | --- |
| `tests/ut/config.test.ts` | 配置默认值、环境变量、namespace policy |
...
6. **namespace policy 要一致**：`isolateUserScopeByAgent` / `isolateAgentScopeByUser` 与服务端不一致会导致写查不一致。
...
- [ ] `X-OpenViking-Agent` 与预期 agent/session 一致。
