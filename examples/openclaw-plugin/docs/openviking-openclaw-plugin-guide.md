...
插件仍会从 OpenClaw runtime/session context 解析一个稳定的 agent id，用于 `peer_role=assistant` 时的 assistant `peer_id`。`peer_prefix` 非空时，assistant peer id 会变成 `<peer_prefix>_<agentId>`；当前实现不发送 `X-OpenViking-Agent` header，也不再使用旧的 `viking://agent/...` 命名空间。
...
4. 按 `recallTargetTypes` 生成搜索计划；默认使用一次 `context_type=memory` 检索当前用户记忆，并在配置了 `peer_role` 时通过 `peer_id` 纳入对应 peer 记忆。显式设为 `resource` 时使用 `context_type=resource`；旧字段 `recallResources=true` 仅在未显式配置 `recallTargetTypes` 时把 `resource` 追加到默认集合。
...
| `viking://user/peers/{peerId}/memories` | 指定 peer 的长期记忆空间 |
...
session 消息和 recall/search 的说话人隔离由 `peer_role` / `peer_prefix` 解析出的 `peer_id` 控制；默认召回使用服务端 `context_type` 过滤，不依赖旧的 `viking://agent/...` 目录。
...
| `peer_role` | session 写入和 recall/search peer 路由 | `none` | 是 | 否 | — | `none` 不写 `peer_id`；`assistant` 使用 runtime agent；`person` 使用 OpenClaw sender 身份：`config.ts:430` |
...
| 同样的 query 在不同 agent / user 命中不一致 | `accountId`、`userId`、`peer_role`、`peer_prefix` | 租户或 `peer_id` 路由不一致 |
...
日志会打印 account/user header 是否设置、target_uri、`peer_id`、query preview、session commit 等信息，但不会打印 apiKey。
...
- 如果未显式配置 `recallTargetTypes`，自动召回默认使用 `context_type=memory` 查当前用户记忆，并在有 `peer_id` 时包含对应 peer 记忆，不会把 resources 文档自动注入；resource-only 用 `recallTargetTypes=["resource"]`，默认记忆 + resources 用 `recallResources=true` 或 `recallTargetTypes=["user","agent","resource"]`。
...
| 不同说话人记忆串用 | `logFindRequests` 中的 `peer_id` | `peer_role` / `peer_prefix` 或 sender 解析不符合预期 |
