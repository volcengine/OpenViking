          ...
          if (!precheck.ok) {
            ...
            try {
              await withTimeout(
                ...
                `memory-openviking: auto-recall timed out after ${AUTO_RECALL_TIMEOUT_MS}ms`,
              );
            } catch (err) {
    ...
    if (cfg.autoCapture) {
      ...
      api.on("agent_end", async (event: { success?: boolean; messages?: unknown[] }, ctx) => {
        ...
        try {
          ...
          const lastCount = lastProcessedMsgCountByAgent.get(agentId) ?? 0;
          const { texts: newTexts, newCount } = extractNewTurnTexts(messages, lastCount);
          lastProcessedMsgCountByAgent.set(agentId, messages.length);
