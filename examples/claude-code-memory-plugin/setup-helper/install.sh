...
# ----- 4. Shell rc wrapper -----
#
# Wrapper body lives in its own file at $WRAPPER_RC_FILE (full overwrite on
# every install — no marker parsing needed inside the wrapper file itself).
# The user's shell rc gets a single one-line source hook (marker-delimited,
# but content never changes, so the marker-replacement logic only runs the
# legacy-cleanup path once when upgrading from a pre-rc-split install).
# Same pattern pyenv / nvm / fnm use.

heading '4. Shell rc — claude function wrapper'

WRAPPER_RC_FILE="${OPENVIKING_CLAUDE_WRAPPER_RC:-$OV_HOME/claude-plugin.rc.sh}"
...
# Write the wrapper body to its standalone rc file (full overwrite).
mkdir -p "$(dirname "$WRAPPER_RC_FILE")"
cat > "$WRAPPER_RC_FILE" <<'WRAPPER'
# OpenViking claude-code memory plugin shell wrapper.
# Generated by examples/claude-code-memory-plugin/setup-helper/install.sh.
# Re-run the installer to update this file; do not hand-edit.
...
WRAPPER
info "Wrote claude() wrapper to $WRAPPER_RC_FILE"

# The user's shell rc gets a single one-line source hook.
