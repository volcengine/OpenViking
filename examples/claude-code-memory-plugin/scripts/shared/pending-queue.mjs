...
export async function replayPending(fetchJSON, log, options = {}) {
  ...
  while (i < pending.length) {
    ...
    if (entry.type === "addMessage") {
      // Background drainers pass consumeRetries:false and require stop-at-first
      // retryable failure with order preserved. Keep that path one-at-a-time;
      // batching is for session-start replay (default consumeRetries:true).
      if (!consumeRetries) {
        ...
        await handleRetryableFailure(claimedFilename, entry);
        deferred += Math.max(0, pending.length - i);
        stop = true;
        continue;
      }
