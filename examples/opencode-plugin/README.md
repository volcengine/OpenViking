...
### Skillset visibility filtering

For skill pools that mix first-party and vendored collections, retrieval can be
scoped by trust domain. Two optional keys filter the skills visible to **both**
retrieval surfaces — the auto-recall context injection and the MCP `find` tool:

```jsonc
{
  "skillsetsOnly": ["local"],        // allowlist: only members of listed sets pass;
                                     // unclassified and future sets are blocked
  "skillsetsExclude": ["jeffreys"]   // denylist: members are dropped; unclassified pass
}
```

Env equivalents: `OPENVIKING_SKILLSETS_ONLY` / `OPENVIKING_SKILLSETS_EXCLUDE`
(comma-separated). A skill is dropped when it belongs to none of the allowlisted
sets (if the allowlist is non-empty — fail-closed, so a newly vendored collection
is invisible until explicitly allowed) or to any excluded set. Both keys may
combine (allowlist minus exceptions). Non-skill URIs (memories, resources)
always pass. When the server-rendered recall block contains blocked skills it is
discarded and re-rendered from the filtered entries; the `find` tool's text
output has the blocked item blocks removed with its header count rewritten.

Membership comes from a local registry mapping skill name to set names,
`~/.openviking/skillsets.json` by default (`OPENVIKING_SKILLSETS_REGISTRY`
overrides the path), regenerated by whatever syncs the skill pool:

```json
{ "skills": { "deploy-runbook": ["local"], "nmap-sweep": ["jeffreys"] } }
```

A missing registry leaves every skill unclassified: an allowlist then blocks all
skills, a denylist blocks none. This is discovery-time filtering only — the
`read` MCP tool still serves any URI the caller names explicitly, so it does not
