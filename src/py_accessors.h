...
public:
    PyDictAccessor(const vdb::Schema& schema) : field_order_(schema.get_field_order()) {}

    bool has_value(const py::dict& row, int field_idx) const {
        const auto& name = field_order_[field_idx].name;
        if (!row.contains(name.c_str())) return false;
    ...
    void write_string(const py::dict& row, int field_idx, char* dest) const {
        std::string s = row[field_order_[field_idx].name.c_str()].cast<std::string>();
    ...
    void write_list_float32(const py::dict& row, int field_idx, char* dest) const {
        py::list l = row[field_order_[field_idx].name.c_str()].cast<py::list>();
        uint16_t len = static_cast<uint16_t>(l.size());
        std::memcpy(dest, &len, 2);
...
public:
    PyObjectAccessor(const vdb::Schema& schema) : field_order_(schema.get_field_order()) {}

    bool has_value(const py::handle& row, int field_idx) const {
        const char* name = field_order_[field_idx].name.c_str();
        if (!py::hasattr(row, name)) return false;
    ...
    int get_string_len(const py::handle& row, int field_idx) const {
        ...
        return row.attr(field_order_[field_idx].name.c_str()).cast<std::string>().length();
    }
    ...
    int get_list_len(const py::handle& row, int field_idx) const {
        py::list l = row.attr(field_order_[field_idx].name.c_str()).cast<py::list>();
        return static_cast<int>(l.size());
    }
    ...
    void write_string(const py::handle& row, int field_idx, char* dest) const {
        std::string s = row.attr(field_order_[field_idx].name.c_str()).cast<std::string>();
        uint16_t len = static_cast<uint16_t>(s.length());
        std::memcpy(dest, &len, 2);
        if (len > 0) std::memcpy(dest + 2, s.data(), len);
    }
    ...
    void write_list_int64(const py::handle& row, int field_idx, char* dest) const {
        py::list l = row.attr(field_order_[field_idx].name.c_str()).cast<py::list>();
        uint16_t len = static_cast<uint16_t>(l.size());
        std::memcpy(dest, &len, 2);
        int64_t* data_ptr = reinterpret_cast<int64_t*>(dest + 2);
    ...
    void write_list_float32(const py::handle& row, int field_idx, char* dest) const {
        py::list l = row.attr(field_order_[field_idx].name.c_str()).cast<py::list>();
        uint16_t len = static_cast<uint16_t>(l.size());
        std::memcpy(dest, &len, 2);
        float* data_ptr = reinterpret_cast<float*>(dest + 2);
    ...
    void write_list_string(const py::handle& row, int field_idx, char* dest) const {
        py::list l = row.attr(field_order_[field_idx].name.c_str()).cast<py::list>();
        uint16_t len = static_cast<uint16_t>(l.size());
        std::memcpy(dest, &len, 2);
        char* cur = dest + 2;
...
inline py::object value_to_py(const vdb::Value& val) {
    if (std::holds_alternative<std::monostate>(val)) return py::none();
    if (std::holds_alternative<int64_t>(val)) return py::cast(std::get<int64_t>(val));
    if (std::holds_alternative<uint64_t>(val)) return py::cast(std::get<uint64_t>(val));
    if (std::holds_alternative<float>(val)) return py::cast(std::get<float>(val));
    if (std::holds_alternative<bool>(val)) return py::cast(std::get<bool>(val));
    if (std::holds_alternative<std::string>(val)) {
        return py::cast(std::get<std::string>(val));
    }
    if (std::holds_alternative<std::vector<int64_t>>(val)) return py::cast(std::get<std::vector<int64_t>>(val));
    if (std::holds_alternative<std::vector<float>>(val)) return py::cast(std::get<std::vector<float>>(val));
    if (std::holds_alternative<std::vector<std::string>>(val)) return py::cast(std::get<std::vector<std::string>>(val));
