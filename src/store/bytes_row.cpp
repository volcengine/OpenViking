...
Schema::Schema(const std::vector<FieldDef>& fields) {
    ...
    for (const auto& field : fields) {
        if (field.id > max_id) max_id = field.id;
    }
    ...
    for (const auto& field : fields) {
        ...
        switch (field.data_type) {
            case FieldType::INT64: byte_len = INT64_SIZE; break;
            case FieldType::UINT64: byte_len = UINT64_SIZE; break;
            case FieldType::FLOAT32: byte_len = FLOAT32_SIZE; break;
            case FieldType::BOOLEAN: byte_len = BOOL_SIZE; break;
...
std::string BytesRow::serialize(const std::vector<Value>& row_data) const {
    ...
    for (size_t i = 0; i < field_order.size(); ++i) {
        ...
        const Value& val = (i < row_data.size() && !std::holds_alternative<std::monostate>(row_data[i])) 
                           ? row_data[i] 
                           : meta.default_value;
        ...
        switch (meta.data_type) {
            ...
            case FieldType::BINARY: {
                 if (std::holds_alternative<std::string>(val)) { // Binary stored as string
                    int len = std::get<std::string>(val).length();
                    var_infos[i] = {variable_region_offset, len};
    ...
    // Be careful with alignment if we were doing raw casting, but we use memcpy so it's fine.
    // However, to match Python's struct.pack behavior which handles endianness and alignment:
    // The Python implementation uses '<' (little endian) and packed without padding except what struct.pack does.
    // Actually struct.pack with format string does strict packing?
    // The Python implementation calculates offsets manually: current_offset += byte_len.
    // So it's tightly packed.
    ...
    for (size_t i = 0; i < field_order.size(); ++i) {
        ...
        switch (meta.data_type) {
            case FieldType::INT64: {
                // FIXED: std::holds_alternative checks for exact type match in variant
                // We should prioritize correct type access
                int64_t v = 0;
                ...
                // In C++, we assume little endian system (usually true for x86/arm)
                // If needed, we can add endian swap. For now assuming little endian.
            ...
            // Variable length fields: write offset to fixed region, then write data to variable region
            case FieldType::STRING:
            case FieldType::BINARY:
            case FieldType::LIST_INT64:
            case FieldType::LIST_FLOAT32:
            case FieldType::LIST_STRING: {
                ...
                if (meta.data_type == FieldType::STRING) {
                    const std::string& s = std::holds_alternative<std::string>(val) ? std::get<std::string>(val) : "";
                    uint16_t len = static_cast<uint16_t>(s.length());
                    std::memcpy(var_ptr, &len, sizeof(len));
                    if (len > 0) std::memcpy(var_ptr + sizeof(len), s.data(), len);
                }
                    ...
                    const auto& vec = std::holds_alternative<std::vector<int64_t>>(val) ? std::get<std::vector<int64_t>>(val) : std::vector<int64_t>{};
                    uint16_t len = static_cast<uint16_t>(vec.size());
                    std::memcpy(var_ptr, &len, sizeof(len));
                    if (len > 0) std::memcpy(var_ptr + sizeof(len), vec.data(), len * sizeof(int64_t));
                }
                else if (meta.data_type == FieldType::LIST_FLOAT32) {
                    const auto& vec = std::holds_alternative<std::vector<float>>(val) ? std::get<std::vector<float>>(val) : std::vector<float>{};
                    uint16_t len = static_cast<uint16_t>(vec.size());
                    std::memcpy(var_ptr, &len, sizeof(len));
                    if (len > 0) std::memcpy(var_ptr + sizeof(len), vec.data(), len * sizeof(float));
                }
                else if (meta.data_type == FieldType::LIST_STRING) {
                    const auto& vec = std::holds_alternative<std::vector<std::string>>(val) ? std::get<std::vector<std::string>>(val) : std::vector<std::string>{};
                    uint16_t len = static_cast<uint16_t>(vec.size());
                    std::memcpy(var_ptr, &len, sizeof(len));
                    var_ptr += sizeof(len);
                    for (const auto& s : vec) {
                        ...
                        if (s_len > 0) std::memcpy(var_ptr, s.data(), s_len);
                        var_ptr += s_len;
                    }
                }
                break;
...
Value BytesRow::deserialize_field(const std::string& serialized_data, const std::string& field_name) const {
    const FieldMeta* meta_ptr = schema_->get_field_meta(field_name);
    if (!meta_ptr) return std::monostate{};
    ...
    switch (meta.data_type) {
        ...
        case FieldType::STRING: {
            ...
            if (offset >= serialized_data.size()) return std::string("");
        ...
        case FieldType::LIST_INT64: {
            uint32_t offset;
            std::memcpy(&offset, field_ptr, sizeof(offset));
            if (offset >= serialized_data.size()) return std::vector<int64_t>{};

            uint16_t len;
            std::memcpy(&len, ptr + offset, sizeof(len));
            
            std::vector<int64_t> vec(len);
            if (len > 0) std::memcpy(vec.data(), ptr + offset + sizeof(len), len * sizeof(int64_t));
            return vec;
        }
        case FieldType::LIST_FLOAT32: {
            uint32_t offset;
            std::memcpy(&offset, field_ptr, sizeof(offset));
            if (offset >= serialized_data.size()) return std::vector<float>{};

            uint16_t len;
            std::memcpy(&len, ptr + offset, sizeof(len));
            
            std::vector<float> vec(len);
            if (len > 0) std::memcpy(vec.data(), ptr + offset + sizeof(len), len * sizeof(float));
            return vec;
        }
        case FieldType::LIST_STRING: {
            uint32_t offset;
            std::memcpy(&offset, field_ptr, sizeof(offset));
            if (offset >= serialized_data.size()) return std::vector<std::string>{};
...
std::map<std::string, Value> BytesRow::deserialize(const std::string& serialized_data) const {
