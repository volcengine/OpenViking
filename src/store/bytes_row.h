        ...
        for (size_t i = 0; i < field_order.size(); ++i) {
            ...
            switch (meta.data_type) {
                case FieldType::INT64: {
                    int64_t v = has_val ? accessor.get_int64(row, i) : std::get<int64_t>(meta.default_value);
                    std::memcpy(field_ptr, &v, sizeof(v));
                    break;
                }
                case FieldType::UINT64: {
                    uint64_t v = has_val ? accessor.get_uint64(row, i) : std::get<uint64_t>(meta.default_value);
                    std::memcpy(field_ptr, &v, sizeof(v));
                    break;
                }
                case FieldType::FLOAT32: {
                    float v = has_val ? accessor.get_float(row, i) : std::get<float>(meta.default_value);
                    std::memcpy(field_ptr, &v, sizeof(v));
                    break;
                }
                case FieldType::BOOLEAN: {
                    bool v = has_val ? accessor.get_bool(row, i) : std::get<bool>(meta.default_value);
                    uint8_t b = v ? 1 : 0;
