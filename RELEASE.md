...
- TypeScript SDK `@openviking/sdk`: for TypeScript/JavaScript HTTP clients.
...
- **TypeScript SDK Release** (`typescript-sdk-release.yml`): triggered by pushing a `typescript-sdk@X.Y.Z` tag, or manually. Publishes `@openviking/sdk` to npm.
- **OpenCode Plugin Release** (`opencode-plugin-release.yml`): runs automatically on pushes to `main` that touch `examples/opencode-plugin/**`, or manually. Publishes `@openviking/opencode-plugin` to npm; skips versions that already exist.
- **Controlplane MCP Release** (`controlplane-mcp-release.yml`): manual only (`workflow_dispatch`). Publishes `mcp-server-openviking-controlplane` (MCP server plus the `ov-cp` CLI).
