...
mod tests {
    ...
    #[test]
    fn test_parse_ignore_dirs_none() {
        let result = parse_ignore_dirs(None);
        assert!(result.is_empty());
    }

    #[test]
    fn test_parse_ignore_dirs_single() {
        let result = parse_ignore_dirs(Some(".git"));
        assert_eq!(result, vec![".git".to_string()]);
    }

    #[test]
    fn test_parse_ignore_dirs_multiple() {
        let result = parse_ignore_dirs(Some(".git,node_modules,dist"));
        assert_eq!(
            result,
            vec![
                ".git".to_string(),
    ...
    #[test]
    fn test_parse_ignore_dirs_trims_whitespace() {
        let result = parse_ignore_dirs(Some(" .git , node_modules , dist "));
        assert_eq!(
            result,
            vec![
    ...
    #[test]
    fn test_parse_ignore_dirs_empty_segments() {
        let result = parse_ignore_dirs(Some(".git,,node_modules,"));
        assert_eq!(
            result,
    ...
    #[test]
    fn test_parse_ignore_dirs_empty_string() {
        let result = parse_ignore_dirs(Some(""));
        assert!(result.is_empty());
    }

    #[test]
    fn test_ignore_dirs_filter_basename_match() {
        ...
        let ignore_list = parse_ignore_dirs(Some(".git"));
        let filter = ignore_dirs_filter(root, &ignore_list);
    ...
    #[test]
    fn test_ignore_dirs_filter_relative_path_match() {
        let temp_dir = tempfile::tempdir().unwrap();
        let root = temp_dir.path();

        fs::create_dir_all(root.join("build").join("css")).unwrap();
        fs::create_dir_all(root.join("src").join("build")).unwrap();
        fs::write(root.join("build/css/style.css"), "body{}").unwrap();
        fs::write(root.join("src/build/debug.o"), "binary").unwrap();
        fs::write(root.join("src/main.rs"), "fn main() {}").unwrap();

        let ignore_list = parse_ignore_dirs(Some("build/"));
        let filter = ignore_dirs_filter(root, &ignore_list);
        ...
        assert!(names.iter().any(|n| n.contains("src/build")));
    }

    #[test]
    fn test_ignore_dirs_filter_relative_path_nested() {
        let temp_dir = tempfile::tempdir().unwrap();
        let root = temp_dir.path();
        ...
        fs::write(root.join("src/build/debug.o"), "binary").unwrap();
        fs::write(root.join("build/output.js"), "js").unwrap();
        fs::write(root.join("src/main.rs"), "fn main() {}").unwrap();

        let ignore_list = parse_ignore_dirs(Some("src/build"));
        let filter = ignore_dirs_filter(root, &ignore_list);
        ...
        assert!(names.iter().any(|n| n.contains("build/output.js")));
        assert!(!names.iter().any(|n| n.contains("src/build")));
    }

    #[test]
    fn test_ignore_dirs_filter_dot_slash_prefix() {
        let temp_dir = tempfile::tempdir().unwrap();
        let root = temp_dir.path();

        fs::create_dir_all(root.join("dist")).unwrap();
        fs::create_dir_all(root.join("src").join("dist")).unwrap();
        fs::write(root.join("dist/bundle.js"), "js").unwrap();
        fs::write(root.join("src/dist/local.js"), "js").unwrap();

        let ignore_list = parse_ignore_dirs(Some("./dist/"));
        let filter = ignore_dirs_filter(root, &ignore_list);
        ...
        assert!(names.iter().any(|n| n.contains("src/dist")));
    }

    #[test]
    fn test_ignore_dirs_filter_mixed_patterns() {
        let temp_dir = tempfile::tempdir().unwrap();
        let root = temp_dir.path();
        ...
        fs::write(root.join("src/build/debug.o"), "bin").unwrap();
        fs::write(root.join("build/output.js"), "js").unwrap();
        fs::write(root.join("src/main.rs"), "fn main() {}").unwrap();

        let ignore_list = parse_ignore_dirs(Some(".git,build/"));
        let filter = ignore_dirs_filter(root, &ignore_list);
    ...
    #[test]
    fn test_ignore_dirs_filter_root_not_pruned() {
        let temp_dir = tempfile::tempdir().unwrap();
        let root = temp_dir.path();
        let root_name = root
            .file_name()
        ...
        let ignore_list = vec![root_name.clone()];
        let filter = ignore_dirs_filter(root, &ignore_list);
        ...
        assert!(
            names.iter().any(|n| n.contains("src")),
            "root directory should not be pruned even if its name matches ignore pattern"
        );
    }
}
